...
namespace
{
  ...
  /// Note that the test only covers the implemented specifications for the preliminary plastic
  /// predictor, i.e., both rotations contributions are taken from the elastic predictor, and
  /// therefore, only the eigenvalues are really interpolated. Further tests should be added when
  /// other alternatives are implemented for the rotational contributions.
  ...
  /// Tests the initialization and the reset procedure for the interpolation point containers used
  ...
  TEST_F(InelasticDefgradFactorsServiceTest, TestAdaptiveEstimateInterpolationManagerStartingPoints)
  {
    ...
    FOUR_C_EXPECT_THROW_WITH_MESSAGE(
        ...
        Core::Exception, "The starting point should not be set using the equivalent stress input");
    ...
    FOUR_C_EXPECT_THROW_WITH_MESSAGE(
        aei_manager_equiv_stress_starting_point.set_starting_point(gp, std::nullopt),
        Core::Exception,
