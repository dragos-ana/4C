...
def retrieve_material_params(sim_input_yaml: Path) -> dict:
    ...
    if "ADAPTIVE_ESTIMATE_INTERP" not in viscoplast_material_params:
        raise ValueError(
            "Could not find ADAPTIVE_ESTIMATE_INTERP section within the viscoplastic material parameters!"
        )
    if (
        "USE_ADAPTIVE_ESTIMATE_INTERP"
        not in viscoplast_material_params["ADAPTIVE_ESTIMATE_INTERP"]
    ):
        raise ValueError(
            "The parameter USE_ADAPTIVE_ESTIMATE_INTERP must be clearly specified within ADAPTIVE_ESTIMATE_INTERP!"
        )

    output_dict = {"LOCAL_NEWTON": viscoplast_material_params["LOCAL_NEWTON"]}
    if viscoplast_material_params["ADAPTIVE_ESTIMATE_INTERP"][
        "USE_ADAPTIVE_ESTIMATE_INTERP"
    ]:
        output_dict["ADAPTIVE_ESTIMATE_INTERP"] = viscoplast_material_params[
            "ADAPTIVE_ESTIMATE_INTERP"
        ]
    return output_dict
...
def check_eval_total_values_in_timint_files(
    ...
    assert data_dict["step"] == list(
        range(1, len(data_dict["step"]) + 1)
    ...
    ), f"Time: {np.array(data_dict['time'])}, Step * Timestep: {np.array(range(1, len(data_dict['step']) + 1)) * data_dict['time'][0]} for {csv_file} "  # we only check single runs, no restarts with different timesteps
    ...
    if not is_summary_file:
        ...
        for x_ind, x in enumerate(data_dict["Eval. iterations (LNL)"]):
            assert (
                int(x) <= local_newton_max_iter * eval_num_of_global_iters[x_ind]
    ...
    if "ADAPTIVE_ESTIMATE_INTERP" in material_params:
        aei_params = material_params["ADAPTIVE_ESTIMATE_INTERP"]
        ...
        if not is_summary_file:
            if "MAX_NUM_PLASTIC_PRED_CONSTRUCT_ITERS" not in aei_params:
                raise ValueError(
                    "MAX_NUM_PLASTIC_PRED_CONSTRUCT_ITERS must be specified within the ADAPTIVE_ESTIMATE_INTERP section of the input file!"
                )
            max_num_ppc_iters = aei_params["MAX_NUM_PLASTIC_PRED_CONSTRUCT_ITERS"]
            for x_ind, x in enumerate(data_dict["Eval. PPC iters (AEI)"]):
                assert (
                    int(x) < max_num_ppc_iters * eval_num_of_global_iters[x_ind]
            ...
            if "MAX_NUM_ESTIMATE_INTERP_ITERS" not in aei_params:
                raise ValueError(
                    "MAX_NUM_ESTIMATE_INTERP_ITERS must be specified within the ADAPTIVE_ESTIMATE_INTERP section of the input file!"
                )
            ...
            if "MAX_NUM_REESTIMATIONS" not in aei_params:
                raise ValueError(
                    "MAX_NUM_REESTIMATIONS must be specified within the ADAPTIVE_ESTIMATE_INTERP section of the input file!"
                )
        ...
        if not is_summary_file:
            if "STARTING_POINT_TYPE" not in aei_params:
                raise ValueError(
                    "STARTING_POINT_TYPE must be specified within ADAPTIVE_ESTIMATE_INTERP!"
                )
            match aei_params["STARTING_POINT_TYPE"]:
                ...
                case "user_set":
                    if "USER_SET_STARTING_POINT" not in aei_params:
                        raise ValueError(
                            "USER_SET_STARTING_POINT must be specified within ADAPTIVE_ESTIMATE_INTERP!"
                        )
...
def check_aei_output_gp_file(
    ...
    if "INTERVAL_SCANNING_PARAM" not in aei_params:
        raise ValueError(
            "INTERVAL_SCANNING_PARAM must be provided within ADAPTIVE_ESTIMATE_INTERP in the simulation input file!"
        )
    ...
    assert set(aei_dict["step"]).issubset(
        set(timint_dict["step"])
    ...
    for i in range(len(aei_dict["step"])):
        ...
        if new_timestep:
            if i > 0:
                assert (
                    int(aei_dict["step"][i]) == int(aei_dict["step"][i - 1]) + 1
        ...
        # verifications if this is not a new timestep and not a new global iteration
            ...
            if int(aei_dict["Local iteration"][i]) != int(
                ...
                assert (
                    lower_interp_bound_updated or updated_current_interp_point
...
def check_lnl_output_gp_file(
    ...
    assert set(lnl_dict["step"]).issubset(
        set(timint_dict["step"])
    ...
    for i in range(len(lnl_dict["step"])):
        ...
        if new_timestep:
            if i > 0:
                assert (
                    int(lnl_dict["step"][i]) == int(lnl_dict["step"][i - 1]) + 1
...
def cli():
    ...
    use_adaptive_estimate_interpolation = "ADAPTIVE_ESTIMATE_INTERP" in material_params
