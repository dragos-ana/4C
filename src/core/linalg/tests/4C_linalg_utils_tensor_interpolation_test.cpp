...
namespace
{
  ...
  TEST(LinalgTensorInterpolationTest, DummyTestForTransvIsotropMaterial)
  {
    std::cout << std::setprecision(16);

    // set input matrix
    Core::LinAlg::Matrix<3, 3> input_matrix{Core::LinAlg::Initialization::zero};
    input_matrix(0, 0) = 1.000000000185222;
    input_matrix(0, 1) = 2.669521426630852e-10;
    input_matrix(0, 2) = 1.570976706534235e-12;
    input_matrix(1, 0) = 2.669521426630814e-10;
    input_matrix(1, 1) = 0.9999999996515426;
    input_matrix(1, 2) = -3.754982987710723e-09;
    input_matrix(2, 0) = 1.570976706534248e-12;
    input_matrix(2, 1) = -3.754982987710434e-09;
    input_matrix(2, 2) = 1.000000000163236;

    std::cout << "input_matrix: " << std::endl;
    input_matrix.print(std::cout);

    // stretch matrix
    Core::LinAlg::Matrix<3, 3> squared_stretch_matrix{Core::LinAlg::Initialization::zero};
    squared_stretch_matrix.multiply_tn(1.0, input_matrix, input_matrix, 0.0);

    // get eigenvalue and eigenvector matrices
    Core::LinAlg::Matrix<3, 3> lambda{Core::LinAlg::Initialization::zero};
    Core::LinAlg::Matrix<3, 3> QT{Core::LinAlg::Initialization::zero};
    Core::LinAlg::syev(squared_stretch_matrix, lambda, QT);
    for (unsigned int i = 0; i < 3; ++i)
    {
      lambda(i, i) = std::sqrt(lambda(i, i));
    }
    Core::LinAlg::Matrix<3, 3> Q{Core::LinAlg::Initialization::zero};
    const double detQ = QT.determinant();
    QT.scale(std::pow(detQ, -1.0 / 3.0));
    Q.update_t(1.0, QT, 0.0);
    ...
    LQ.multiply_nn(1.0, lambda, Q, 0.0);
    Core::LinAlg::Matrix<3, 3> QTLQ{Core::LinAlg::Initialization::zero};
    QTLQ.multiply_tn(1.0, Q, LQ, 0.0);
    Core::LinAlg::Matrix<3, 3> inv_QTLQ{Core::LinAlg::Initialization::zero};
    inv_QTLQ.invert(QTLQ);
    Core::LinAlg::Matrix<3, 3> rot_matrix{Core::LinAlg::Initialization::zero};
    rot_matrix.multiply_nn(1.0, input_matrix, inv_QTLQ, 0.0);

    std::cout << "rot_matrix: " << std::endl;
    rot_matrix.print(std::cout);

    // rotation vector associated to R, Q
    Core::LinAlg::Matrix<3, 1> rot_vect_R = Core::LinAlg::calc_rot_vect_from_rot_matrix(rot_matrix);
    Core::LinAlg::Matrix<3, 1> rot_vect_Q = Core::LinAlg::calc_rot_vect_from_rot_matrix(Q);

    std::cout << "rot_vect_R: " << std::endl;
    rot_vect_R.print(std::cout);
    std::cout << "rot_vect_Q: " << std::endl;
    rot_vect_Q.print(std::cout);

    // reconstruct the input matrix
    Core::LinAlg::Matrix<3, 3> input_matrix_reconstructed{Core::LinAlg::Initialization::zero};
    input_matrix_reconstructed.multiply_nn(1.0, rot_matrix, QTLQ, 0.0);


    std::cout << "LQ: " << std::endl;
    LQ.print(std::cout);
    std::cout << "QTLQ: " << std::endl;
    QTLQ.print(std::cout);
    std::cout << "input_matrix_reconstructed: " << std::endl;
    input_matrix_reconstructed.print(std::cout);


    // perform polar decomposition with the implemented function
    Core::LinAlg::Matrix<3, 3> stretch_matrix_polar_decomp{Core::LinAlg::Initialization::zero};
    Core::LinAlg::Matrix<3, 3> rot_matrix_polar_decomp{Core::LinAlg::Initialization::zero};
    Core::LinAlg::Matrix<3, 3> lambda_matrix_polar_decomp{Core::LinAlg::Initialization::zero};
    std::array<std::pair<double, Core::LinAlg::Matrix<3, 1>>, 3> spectral_pairs_polar_decomp;
    Core::LinAlg::matrix_3x3_polar_decomposition(input_matrix, rot_matrix_polar_decomp,
        stretch_matrix_polar_decomp, lambda_matrix_polar_decomp, spectral_pairs_polar_decomp);
    Core::LinAlg::Matrix<3, 3> input_matrix_reconstructed_polar_decomp{
        Core::LinAlg::Initialization::zero};
    input_matrix_reconstructed_polar_decomp.multiply_nn(
        rot_matrix_polar_decomp, stretch_matrix_polar_decomp);


    std::cout << "rot_matrix_polar_decomp: " << std::endl;
    rot_matrix_polar_decomp.print(std::cout);
    std::cout << "stretch_matrix_polar_decomp: " << std::endl;
    stretch_matrix_polar_decomp.print(std::cout);
    std::cout << "lambda_matrix_polar_decomp: " << std::endl;
    lambda_matrix_polar_decomp.print(std::cout);
    std::cout << "input_matrix_reconstructed_polar_decomp: " << std::endl;
    input_matrix_reconstructed_polar_decomp.print(std::cout);
    std::cout << "spectral_pairs_polar_decomp: " << std::endl;
    for (auto sp : spectral_pairs_polar_decomp)
    {
