    ...
    std::shared_ptr<Core::FE::Discretization> discretization)
{
  ...
  if ((s2ikinetics_conditions.size() + s2ikinetics_growth_conditions.size() +
          s2isclcoupling_condition.size()) !=
      (s2imeshtying_conditions.size() + s2inoevaluation_conditions.size()))
  {
    FOUR_C_THROW(
        "For each 'S2IKinetics' or 'S2IKineticsGrowth' or 'S2ISCLCoupling' condition a "
        "corresponding 'S2IMeshtying' or "
    ...
    const Core::LinAlg::Vector<double>& state, const int scatra_dofid)
{
  ...
  for (auto& it_node : nodesToReconstruct)
  {
    ...
    for (int col = 0; col < numcol; col++)
    {
      ...
      gradphirow->get_vector(col).get_values()[lid] = node_gradphi_smoothed(col, 0);
    }
  }  // end loop over nodes
    ...
    std::shared_ptr<Core::FE::Discretization> discretization)
{
  ...
  for (auto& kinetics_condition : s2ikinetics_conditions)
  {
    ...
    if (side == S2I::side_source)
    {
      // determine whether this condition should model simplified growth
      if (kinetics_condition->parameters().get_or<bool>("MODEL_SIMPLIFIED_GROWTH", false))
      {
        if (kinetics_condition->parameters().get_if<S2I::KineticModels>("KINETIC_MODEL") !=
                nullptr &&
            kinetics_condition->parameters().get<S2I::KineticModels>("KINETIC_MODEL") ==
