...
void ScaTra::MeshtyingStrategyS2I::evaluate_meshtying()
{
  ...
  switch (couplingtype_)
  {
    case S2I::coupling_matching_nodes:
    {
      ...
      for (const auto* const condition :
          kinetics_conditions_meshtying_slaveside_ | std::views::values)
      {
        if (condition->parameters().get<S2I::KineticModels>("KINETIC_MODEL") !=
                static_cast<int>(S2I::kinetics_nointerfaceflux) and
            condition->g_type() != Core::Conditions::geometry_type_point)
        {
          // collect condition specific data and store to scatra boundary parameter class
          set_condition_specific_scatra_parameters(*condition);
          ...
          if (not slaveonly_)
          {
            ...
            // if we have simplified growth conditions: integrate ODE for d_Li
            if (condition->parameters().get_or<bool>("MODEL_SIMPLIFIED_GROWTH", false))
            {
              if (condition->parameters().get<S2I::KineticModels>("KINETIC_MODEL") !=
      ...
      switch (matrixtype_)
      {
        case Core::LinAlg::MatrixType::sparse:
        {
          ...
          else if (!scatratimint_->discretization()->has_condition("PointCoupling"))
          {
            ...
            Core::LinAlg::SparseMatrix systemmatrixrowsslave(*icoup_->source_dof_map(), 81);

            // extract slave-side rows of system matrix into temporary matrix
            extract_matrix_rows(*systemmatrix, systemmatrixrowsslave, *icoup_->source_dof_map());

            // zero out slave-side rows of system matrix and put a one on the main diagonal
            systemmatrix->complete();
            systemmatrix->apply_dirichlet(*icoup_->source_dof_map(), true);
            systemmatrix->un_complete();

            // loop over all slave-side rows of system matrix
            for (int slavedoflid = 0; slavedoflid < icoup_->source_dof_map()->num_my_elements();
                ++slavedoflid)
            {
            ...
            systemmatrixrowsslave.complete(
                *scatratimint_->dof_row_map(), *icoup_->source_dof_map());
        ...
        case Core::LinAlg::MatrixType::block_condition_dof:
        {
          ...
          {
            auto ksm(
                std::make_shared<Core::LinAlg::SparseMatrix>(*icoup_->source_dof_map(), 81, false));
            auto kms(
      ...
      else if (!scatratimint_->discretization()->has_condition("PointCoupling"))
      {
        ...
        Core::LinAlg::Vector<double> residualslave(*icoup_->source_dof_map());
        ...
        interfacemaps_->add_vector(
            *icoup_->source_to_target(residualslave), 2, *scatratimint_->residual());
      }
    ...
    case S2I::coupling_nts_standard:
    {
      ...
      for (auto& [condition_id, condition] : kinetics_conditions_meshtying_slaveside_)
      {
        // extract mortar interface discretization
        Core::FE::Discretization& idiscret = icoupmortar_[condition_id]->interface()->discret();
          ...
          switch (matrixtype_)
          {
            case Core::LinAlg::MatrixType::sparse:
            {
              ...
              // interface layer thicknesses
              {
                ...
                auto islavematrix =
                    std::make_shared<Core::LinAlg::SparseMatrix>(*(icoup_)->source_dof_map(), 81);
...
/*--------------------------------------------------------------------------------------*
 *--------------------------------------------------------------------------------------*/
void ScaTra::MeshtyingStrategyS2I::evaluate_and_assemble_capacitive_contributions() const
{
  // create parameter list for elements
  ...
  for (const auto* const condition : kinetics_conditions_meshtying_slaveside_ | std::views::values)
  {
    if (condition->parameters().get<S2I::KineticModels>("KINETIC_MODEL") ==
        static_cast<int>(S2I::kinetics_butlervolmerreducedcapacitance))
    {
      ...
      scatratimint_->discretization()->evaluate_condition(capcondparas, islavematrix_,
          ...
          "S2IKinetics", condition->parameters().get<int>("ConditionID"));
    }
  }
  ...
  interfacemaps_->add_vector(*islaveresidual_, 1, *scatratimint_->residual());
  // transform master residuals and assemble into global residual vector
  interfacemaps_->add_vector(*icoup_->source_to_target(*imasterresidual_on_slave_side), 2,
      *scatratimint_->residual(), 1.0);
}
    ...
    const Teuchos::ParameterList& params, ScaTra::MortarCellAssemblyStrategy& strategy) const
{
  ...
  for (const auto& [integration_cell, impl_type] : cells)
  {
    ...
    evaluate_mortar_cell(idiscret, *cell, impl_type, *slaveelement, *masterelement, la_slave,
    ...
    const S2I::InterfaceSides vector2_side) const
{
  ...
  for (int inode = 0; inode < noderowmap_slave.num_my_elements(); ++inode)
  {
    ...
    evaluate_slave_node(idiscret, *slavenode, islavenodeslumpedareas.local_values_as_span()[inode],
        static_cast<ScaTra::ImplType>(islavenodesimpltypes.get_local_values()[inode]),
        *slaveelement, *masterelement, la_slave, la_master, params, strategy.cell_matrix1(),
    ...
    const S2I::InterfaceSides vector2_side) const
{
  ...
  for (int ielement = 0; ielement < ielecolmap.num_my_elements(); ++ielement)
  {
    ...
    evaluate_mortar_element(idiscret, *element,
        static_cast<ScaTra::ImplType>(ieleimpltypes.get_local_values()[ielement]), la, params,
...
void ScaTra::MeshtyingStrategyS2I::setup_meshtying()
{
  ...
  for (const auto* s2imeshtying_cond : s2imeshtying_conditions)
  {
    for (const auto* s2ikinetics_cond : s2ikinetics_conditions)
    {
      ...
      switch (s2ikinetics_cond_interface_side)
      {
        ...
        {
          if (!kinetics_conditions_meshtying_slaveside_.contains(s2ikinetics_cond_id))
          {
              ...
              static_cast<int>(S2I::kinetics_butlervolmerreducedcapacitance))
          {
            ...
            if (timeintscheme != ScaTra::timeint_bdf2 &&
  ...
  switch (couplingtype_)
  {
    ...
    case S2I::coupling_matching_nodes:
    {
      ...
      if (independent_setup_of_conditions_)
      {
        ...
        for (auto& [kineticsID, kinetics_condition] : kinetics_conditions_meshtying_slaveside_)
        {
      ...
      else
      {
        ...
        for (const auto& [kineticsID, kinetics_condition] :
            kinetics_conditions_meshtying_slaveside_)
        {
          if (num_dof_per_condition == -1)
          {
      ...
      auto ifullmap = Core::LinAlg::merge_map(icoup_->source_dof_map(), icoup_->target_dof_map());
      ...
      imaps.emplace_back(icoup_->source_dof_map());
      imaps.emplace_back(icoup_->target_dof_map());
      ...
      islaveresidual_ = std::make_shared<Core::LinAlg::Vector<double>>(*(icoup_->source_dof_map()));
    ...
    case S2I::coupling_nts_standard:
    {
      ...
      for (auto& [condid, slave_cond] : kinetics_conditions_meshtying_slaveside_)
      {
        ...
        std::vector<const Core::Conditions::Condition*> slavecondition(1, slave_cond);
        ...
        if (couplingtype_ != S2I::coupling_nts_standard)
        {
          ...
          Core::LinAlg::Vector<int> impltypes_row(*interface.source_row_elements());
          for (int iele = 0; iele < interface.source_row_elements()->num_my_elements(); ++iele)
          {
            impltypes_row.get_local_values()[iele] =
                dynamic_cast<const Discret::Elements::Transport*>(
                    std::dynamic_pointer_cast<const Core::Elements::FaceElement>(
                        slave_cond->geometry().at(interface.source_row_elements()->gid(iele)))
                        ->parent_element())
                    ->impl_type();
          }
          ...
          Core::LinAlg::Vector<int> impltypes_col(*interface.source_col_elements());
        ...
        else
        {
          ...
          // extract slave-side noderowmap
          const Core::LinAlg::Map& noderowmap_slave = *interface.source_row_nodes();
          ...
          for (int inode = 0; inode < noderowmap_slave.num_my_elements(); ++inode)
          {
            ...
            interface.find_target_elements(*slavenode, master_mortar_elements);
          ...
          const Core::LinAlg::Map& elecolmap_slave = *interface.source_col_elements();
          ...
          for (int ielement = 0; ielement < elecolmap_slave.num_my_elements(); ++ielement)
          {
            ...
            islaveelementsimpltypes.get_local_values()[ielement] =
                dynamic_cast<Discret::Elements::Transport*>(
                    std::dynamic_pointer_cast<Core::Elements::FaceElement>(
                        slave_cond->geometry().at(elecolmap_slave.gid(ielement)))
                        ->parent_element())
                    ->impl_type();
          }
        ...
        imastermap = Core::LinAlg::merge_map(imastermap, interface.target_row_dofs(), false);
        islavemap = Core::LinAlg::merge_map(islavemap, interface.source_row_dofs(), false);
        ifullmap = Core::LinAlg::merge_map(ifullmap,
            Core::LinAlg::merge_map(
                interface.target_row_dofs(), interface.source_row_dofs(), false),
            false);
        if (imortarredistribution_)
        {
          imastermap_ = Core::LinAlg::merge_map(imastermap_, icoupmortar.target_dof_map(), false);
          islavemap_ = Core::LinAlg::merge_map(islavemap_, icoupmortar.source_dof_map(), false);
        }
      }
...
void ScaTra::MeshtyingStrategyS2I::add_time_integration_specific_vectors() const
{
  ...
  if (couplingtype_ == S2I::coupling_matching_nodes)
  {
    ...
    interfacemaps_->insert_vector(
        *icoup_->target_to_source(*interfacemaps_->extract_vector(*(scatratimint_->phiafnp()), 2)),
    ...
    if (has_capacitive_contributions_)
    {
      ...
      interfacemaps_->insert_vector(*icoup_->target_to_source(*interfacemaps_->extract_vector(
                                        *(scatratimint_->phidtnp()), 2)),
...
void ScaTra::MeshtyingStrategyS2I::init_meshtying()
{
  ...
  if (conditions.size())
  {
    ...
    {
      if (conditioned_node_ids.contains(node_gid))
      {
        auto node_lid = col_map.lid(node_gid);
        if (node_lid >= 0)
        {
          // add one degree of freedom for scatra-scatra interface layer thickness to current node
          numdofpernode->get_local_values()[node_lid] = 1;
        }


        FOUR_C_ASSERT_ALWAYS(
            static_cast<size_t>(node_lid) < numdofpernode->get_local_values().size(),
...
template <Core::FE::CellType distype_s, Core::FE::CellType distype_m>
double ScaTra::MortarCellCalc<distype_s, distype_m>::eval_shape_func_and_dom_int_fac_at_int_point(
    const Mortar::Element& element, const Core::FE::IntPointsAndWeights<nsd_slave_>& intpoints,
    const int iquad)
{
    ...
    Core::LinAlg::SerialDenseVector& r_s, Core::LinAlg::SerialDenseVector& r_m)
{
  ...
  // always in contact
  constexpr double pseudo_contact_fac = 1.0;
...
void ScaTra::MortarCellCalc<distype_s, distype_m>::evaluate_nodal_area_fractions(
    const Mortar::Element& slaveelement, Core::LinAlg::SerialDenseVector& areafractions)
{
