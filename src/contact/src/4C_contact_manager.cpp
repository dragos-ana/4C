...
void CONTACT::Manager::postprocess_quantities(Core::IO::DiscretizationWriter& output)
{
  ...
  std::shared_ptr<Core::LinAlg::Vector<double>> normal_traction_exp =
      std::make_shared<Core::LinAlg::Vector<double>>(*problemdofs);
  Core::LinAlg::export_to(*normal_traction, *normal_traction_exp);

  // tangential plane
  std::shared_ptr<const Core::LinAlg::Vector<double>> tangential_traction =
      get_strategy().contact_tangential_traction();
  std::shared_ptr<Core::LinAlg::Vector<double>> tangential_traction_exp =
      std::make_shared<Core::LinAlg::Vector<double>>(*problemdofs);
  Core::LinAlg::export_to(*tangential_traction, *tangential_traction_exp);

  // write to output
  // contact tractions in normal and tangential direction
  output.write_vector("normal_contact_traction", normal_traction_exp);
  output.write_vector("tangential_contact_traction", tangential_traction_exp);
