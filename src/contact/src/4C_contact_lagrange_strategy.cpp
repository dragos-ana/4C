...
void CONTACT::LagrangeStrategy::compute_contact_tractions()
{
  ...
  if (nonSmoothContact_)
  {
    ...
    d_matrix()->multiply(true, *normal_traction_, *forcenormal_);
    forcetangential_ = std::make_shared<Core::LinAlg::Vector<double>>(source_dof_row_map(true));
    d_matrix()->multiply(true, *tangential_traction_, *forcetangential_);

    Core::LinAlg::Vector<double> forcenormal(source_dof_row_map(true));
    d_matrix()->multiply(true, *normal_traction_, forcenormal);

    Core::LinAlg::Vector<double> forcetangential(source_dof_row_map(true));
    d_matrix()->multiply(true, *tangential_traction_, forcetangential);
    ...
    tangential_traction_->update(1.0, forcetangential, 0.0);
    normal_traction_->update(1.0, forcenormal, 0.0);
    ...
    std::shared_ptr<Core::LinAlg::Vector<double>>& feff)
{
  ...
  if (systype_ == CONTACT::SystemType::condensed)
  {
    // double-check if this is a dual LM system
    if (shapefcn != Mortar::shape_dual && shapefcn != Mortar::shape_petrovgalerkin &&
        Teuchos::getIntegralValue<Mortar::LagMultQuad>(params(), "LM_QUAD") !=
    ...
    std::shared_ptr<Core::LinAlg::SparseMatrix> kteff, Core::LinAlg::Vector<double>& rhs)
{
  ...
  // double-check if this is a dual LM system
  if (shapefcn != Mortar::shape_dual && shapefcn != Mortar::shape_petrovgalerkin &&
      Teuchos::getIntegralValue<Mortar::LagMultQuad>(params(), "LM_QUAD") != Mortar::lagmult_const)
    FOUR_C_THROW("Condensation only for dual LM");
