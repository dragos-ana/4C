...
/*----------------------------------------------------------------*
 |  Assemble source coordinates (xs)                 gitterle 10/09|
 *----------------------------------------------------------------*/
void CONTACT::Interface::assemble_source_coord(std::shared_ptr<Core::LinAlg::Vector<double>>& xsmod)
{
...
void CONTACT::Interface::assemble_reg_tangent_forces_penalty()
{
  ...
  for (int i = 0; i < source_row_nodes()->num_my_elements(); ++i)
  {
    ...
    Core::LinAlg::SerialDenseMatrix delta_tangential_traction(numdof, 1);
    Core::LinAlg::multiply(
        0.0, delta_tangential_traction, kappa * pptan, tangential_projection_matrix, jumpvec);

    // get tangential traction $\boldsymbol{t}_{\tau,n}$ from the previous time instant
    Core::LinAlg::SerialDenseMatrix traction_old(numdof, 1);
    for (int i = 0; i < numdof; i++)
    {
      traction_old(i, 0) = cnode->fri_data().tractionold()[i];
    }
    //  project previous tangential traction onto the current tangential plane: this is a simplified
    //  approach of ensuring that the tangential traction is always oriented tangential to the
    //  current surface. The approach is not fully consistent, since projecting can change the
    //  magnitude of the previous tangential traction; but it is cheap, and should be fine for small
    //  incremental changes of the surface normal.
    Core::LinAlg::SerialDenseMatrix projected_tangential_traction_old(numdof, 1);
    Core::LinAlg::multiply(
        0.0, projected_tangential_traction_old, 1.0, tangential_projection_matrix, traction_old);


    // evaluate trial tangential traction \f$ \boldsymbol{t}_{\tau,\text{trial}} =
    // \boldsymbol{t}_{\tau, n} + \Delta \boldsymbol{t}_{\tau,\text{trial}}  \f$
    std::vector<double> trial_tangential_traction(numdof);
    double magnitude_trial_tangential_traction = 0;
    for (int i = 0; i < numdof; i++)
    {
      trial_tangential_traction[i] =
          projected_tangential_traction_old(i, 0) + delta_tangential_traction(i, 0);
      magnitude_trial_tangential_traction +=
          (trial_tangential_traction[i] * trial_tangential_traction[i]);
    }
    magnitude_trial_tangential_traction = sqrt(magnitude_trial_tangential_traction);
    ...
    {
      // set stick node status
      cnode->fri_data().slip() = false;

      // in the stick case, traction is \f$ \boldsymbol{t}_{\tau,n+1} =
      ...
      for (int i = 0; i < numdof; i++)
      {
        ...
        cnode->mo_data().lm()[i] = n[i] * (-kappa * ppnor * gap) + cnode->fri_data().traction()[i];
      }
    }
    else
    {
      // set slip node status
      cnode->fri_data().slip() = true;

      // in the slip case, tangential traction is evaluated with a return mapping algorithm: \f$
      // \boldsymbol{t}_{\tau} = t_{\tau,\text{max}}
      ...
      for (int dimrow = 0; dimrow < numdof; ++dimrow)
      {
        for (int dim = 0; dim < numdof; ++dim)
        {
          ...
          for (colcurr = derivjump[dim].begin(); colcurr != derivjump[dim].end(); ++colcurr)
          {
            ...
            double val =
                pptan * kappa * (colcurr->second) * tangential_projection_matrix(dimrow, dim);
    ...
    else if (cnode->active() == true && cnode->fri_data().slip() == true)
    {
      ...
      for (int dimrow = 0; dimrow < numdof; ++dimrow)
      {
        for (int dim = 0; dim < numdof; ++dim)
        {
          ...
          {
            int col = colcurr->first;
            double val = pptan * kappa * (colcurr->second) *
      ...
      for (int j = 0; j < numdof; ++j)
      {
        for (gcurr = derivg.begin(); gcurr != derivg.end(); ++gcurr)
        {
          cnode->add_deriv_z_value(j, gcurr->first,
              -frcoeff * kappa * ppnor * (gcurr->second) * trial_tangential_traction[j] /
                  magnitude_trial_tangential_traction);
        }
      }
      ...
      for (int dimout = 0; dimout < numdof; ++dimout)
      {
        ...
        traction += projected_tangential_traction_old(dimout, 0);
        ...
        for (int dim = 0; dim < numdof; ++dim)
        {
          ...
          {
            int col = colcurr->first;
            double val = tangential_projection_matrix(dimout, dim) * pptan * kappa *
                         (colcurr->second) * traction / magnitude_trial_tangential_traction;
        ...
        for (_colcurr = derivn[dimout].begin(); _colcurr != derivn[dimout].end(); ++_colcurr)
        {
          ...
          for (int dim = 0; dim < numdof; ++dim)
          {
            double val = -_colcurr->second * n[dim] * cnode->fri_data().jump()[dim] * traction /
                         magnitude_trial_tangential_traction * pptan * kappa;
            for (int dimrow = 0; dimrow < numdof; ++dimrow)
            {
              double val1 = val * temp[dimrow];
              cnode->add_deriv_z_value(dimrow, col, val1);
...
void CONTACT::Interface::assemble_reg_tangent_forces_uzawa()
{
  ...
  for (int i = 0; i < source_row_nodes()->num_my_elements(); ++i)
  {
    ...
    Core::LinAlg::SerialDenseMatrix delta_tangential_traction(dim, 1);
    Core::LinAlg::multiply(
        0.0, delta_tangential_traction, kappa * pptan, tangential_projection_matrix, jumpvec);

    // Evaluate trial tangential traction: $ \boldsymbol{t}_{\tau,\text{trial}} $
    std::vector<double> trial_tangential_traction(dim);
    double magnitude = 0;
    for (int i = 0; i < dim; i++)
    {
      trial_tangential_traction[i] = lmuzawatan(i, 0) + delta_tangential_traction(i, 0);
      magnitude += (trial_tangential_traction[i] * trial_tangential_traction[i]);
    }
             ...
             ((abs(maxtantrac) - magnitude >= 0) or ftype == CONTACT::FrictionType::stick))
    {
      ...
      for (int j = 0; j < dim; ++j)
        cnode->mo_data().lm()[j] =
            n[j] * (lmuzawan - kappa * ppnor * gap) + trial_tangential_traction[j];
    }
    else
    {
      ...
      // compute lagrange multipliers and store into node
      for (int j = 0; j < dim; ++j)
        cnode->mo_data().lm()[j] = n[j] * (lmuzawan - kappa * ppnor * gap) +
                                   trial_tangential_traction[j] * maxtantrac / magnitude;
    }
    ...
    else if (cnode->active() == true && cnode->fri_data().slip() == true)
    {
      ...
      for (int dimrow = 0; dimrow < numdof; ++dimrow)
      {
        for (int dim = 0; dim < numdof; ++dim)
        {
          ...
          for (colcurr = derivjump[dim].begin(); colcurr != derivjump[dim].end(); ++colcurr)
          {
            int col = colcurr->first;
            double val = pptan * kappa * (colcurr->second) *
                         tangential_projection_matrix(dimrow, dim) * maxtantrac / magnitude;
            cnode->add_deriv_z_value(dimrow, col, val);
          }
        }
      }
        ...
        for (_colcurr = (derivn[j]).begin(); _colcurr != (derivn[j]).end(); ++_colcurr)
        {
          for (int k = 0; k < numdof; ++k)
          {
            double val = frcoeff * (_colcurr->second) * lmuzawa(j, 0) *
                         trial_tangential_traction[k] / magnitude;
      ...
      std::vector<double> temp(numdof);
      for (int dim = 0; dim < numdof; ++dim)
        temp[dim] = -maxtantrac / (magnitude * magnitude) * trial_tangential_traction[dim];
      ...
      for (int dimout = 0; dimout < numdof; ++dimout)
      {
        double traction = 0;
        for (int dim = 0; dim < numdof; ++dim)
          traction += tangential_projection_matrix(dimout, dim) *
        ...
        for (int dim = 0; dim < numdof; ++dim)
        {
          ...
          for (colcurr = derivjump[dim].begin(); colcurr != derivjump[dim].end(); ++colcurr)
          {
            int col = colcurr->first;
            double val = tangential_projection_matrix(dimout, dim) * pptan * kappa *
                         (colcurr->second) * traction / magnitude;
...
{
  // loop over all source nodes (row map)
    ...
    std::shared_ptr<Core::LinAlg::SparseMatrix> nglobal)
{
  ...
  // loop over all active source nodes of the interface
...
void CONTACT::Interface::assemble_lin_d(Core::LinAlg::SparseMatrix& lindglobal, bool usePoroLM)
{
  ...
  /**********************************************************************/
  // we have: D_jk,c with j = Lagrange multiplier source dof
  //                 with k = Displacement source dof
  //                 with c = Displacement source or target dof
...
void CONTACT::Interface::assemble_lin_m(Core::LinAlg::SparseMatrix& linmglobal, bool usePoroLM)
{
  ...
  // NEW VERSION (09/2010): No more communication, thanks to FE_MATRIX!
  /**********************************************************************/
  // we have: M_jl,c with j = Lagrange multiplier source dof
  //                 with l = Displacement target dof
  //                 with c = Displacement source or target dof
  ...
  for (int j = 0; j < snoderowmap_->num_my_elements(); ++j)
  {
    ...
    // Mortar matrix D and M derivatives
    std::map<int, std::map<int, double>>& target_deriv = cnode->data().get_deriv_m();
    ...
    std::map<int, std::map<int, double>>::iterator t_curr = target_deriv.begin();
...
void CONTACT::Interface::assemble_g(Core::LinAlg::Vector<double>& gglobal)
{
  ...
  for (int i = 0; i < snoderowmap_->num_my_elements(); ++i)
  {
    ...
    if (cnode->data().getg() != 0.0)
    {
      ...
      // (e.g. patch tests with source side being wider than target side).
  ...
  if (ftype == CONTACT::FrictionType::tresca)
  {
    ...
    for (int i = 0; i < slipnodes_->num_my_elements(); ++i)
    {
      ...
      else
      {
        ...
        std::set<int> target_nodes_current = cnode->fri_data().get_target_nodes();
        std::set<int> target_nodes_old = cnode->fri_data().get_target_nodes_old();
        ...
        for (t_curr = target_nodes.begin(); t_curr != target_nodes.end(); t_curr++)
        {
          ...
          // compute linstick-matrix entry of the current active node / target node pair
          // loop over all derivative maps (=dimensions)
          for (int dim = 0; dim < cnode->num_dof(); ++dim)
          {
            int col = c_target_node->dofs()[dim];
        ...
        // loop over all target nodes in the DerivM-map of the active source node
        for (dmcurr = dmmap.begin(); dmcurr != dmmap.end(); ++dmcurr)
        {
          ...
          double tdotx = 0.0;
          for (int dim = 0; dim < cnode->num_dof(); ++dim) tdotx += txi[dim] * target_xi[dim];

          // compute entry of the current active node / target node pair
    ...
    Coupling::Adapter::Coupling& coupfs, bool AssembleVelocityLin)
{
  ...
  std::shared_ptr<const Core::LinAlg::Map> TargetDofMap_full;
  std::shared_ptr<const Core::LinAlg::Map> PermSourceDofMap_full;
  ...
  for (int i = 0; i < activenodes_->num_my_elements(); ++i)
  {
    ...
    {  // Assign fluid velocity linearization to matrix
      ...
      for (colcurr = dgmap.begin(); colcurr != dgmap.end(); ++colcurr)
      {
        int col = PermSourceDofMap_full->gid(TargetDofMap_full->lid(colcurr->first));
        double val = colcurr->second;
      ...
      {
        int col =
            PermSourceDofMap_full->gid(TargetDofMap_full->lid(colcurr->first)) + Interface::n_dim();
        double val = colcurr->second;
...
/*------------------------------------------------------------------------*
 | Derivative of D-matrix multiplied with a source dof vector              |
 *------------------------------------------------------------------------*/
...
{
  // we have: D_jk,c with j = Source dof
  //                 with k = Displacement source dof
  //                 with c = Displacement source or target dof
  ...
  for (int j = 0; j < snoderowmap_->num_my_elements(); ++j)
  {
    ...
    /********************************************** LinDMatrix **********/
    // loop over all DISP source nodes in the DerivD-map of the current source node
    for (int k = 0; k < sourcesize; ++k)
    {
      int s_gid = s_curr->first;
      ++s_curr;
      ...
      Core::Nodes::Node* source_node = idiscret_->g_node(s_gid);
      if (!source_node) FOUR_C_THROW("Cannot find node with gid %", s_gid);
      Node* c_source_node = dynamic_cast<Node*>(source_node);  // current source node

      // Mortar matrix D derivatives
      std::map<int, double>& thisdderiv = cnode->data().get_deriv_d()[s_gid];
      int mapsize = (int)(thisdderiv.size());

      if (cnode->num_dof() != c_source_node->num_dof())
      ...
      for (int prodj = 0; prodj < cnode->num_dof(); ++prodj)
      {
        ...
        {
          int col = scolcurr->first;
          int sourcedofgid = cnode->dofs()[prodj];

          int sourcedoflid = x->get_map().lid(sourcedofgid);
          if (sourcedoflid < 0) FOUR_C_THROW("invalid source dof lid");
          double val = x->local_values_as_span()[sourcedoflid] * (scolcurr->second);
    ...
    if (s_curr != dderiv.end())
      FOUR_C_THROW("AssembleCoupLin: Not all connected source nodes to Lin(D*z_s) considered!");
  }
...
/*-----------------------------------------------------------------------------------*
 | Derivative of transposed M-matrix multiplied with a source dof vector  seitz 01/18 |
 *-----------------------------------------------------------------------------------*/
void CONTACT::Interface::assemble_coup_lin_m(
    Core::LinAlg::SparseMatrix& CoupLin, const std::shared_ptr<Core::LinAlg::Vector<double>> x)
{
  // we have: M_jl,c with j = Source dof
  //                 with l = Displacement target dof
  //                 with c = Displacement source or target dof
  ...
  for (int j = 0; j < snoderowmap_->num_my_elements(); ++j)
  {
    ...
    for (int l = 0; l < targetsize; ++l)
    {
      ...
      if (cnode->num_dof() != c_target_node->num_dof())
        FOUR_C_THROW("Mortar Nodes on interface must have same number of dofs!");

      // inner product M_{jl,c} * z_j for index j
      for (int prodj = 0; prodj < c_target_node->num_dof(); ++prodj)
      {
        int row = c_target_node->dofs()[prodj];
        std::map<int, double>::iterator mcolcurr = thismderiv.begin();
        ...
        for (int c = 0; c < mapsize; ++c)
        {
          ...
          int sourcedoflid = x->get_map().lid(sourcedofgid);
          double val = (*x).local_values_as_span()[sourcedoflid] * (mcolcurr->second);
    ...
    }  // loop over all target nodes, connected to this source node => Lin(M*z_s) finished

    // check for completeness of DerivM-Target-iteration
    if (t_curr != target_deriv.end())
      FOUR_C_THROW("AssembleCoupLin: Not all target entries of Lin(M*z_s) considered!");
