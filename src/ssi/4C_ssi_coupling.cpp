    ...
    std::shared_ptr<Core::FE::Discretization> structdis, std::shared_ptr<SSI::SSIBase> ssi_base)
{
  ...
  if (scatra_integrator->has_simplified_growth_conditions())
  {
    scatra_integrator->init_simplified_growth_dofsets();
    scatra_dofset_counter += 3;
    if (structdis->add_dof_set(scatradis->get_dof_set_proxy(scatra_integrator->nds_growth())) !=
        ++structure_dofset_counter)
      FOUR_C_THROW("unexpected dof sets in structure field");
    if (structdis->add_dof_set(scatradis->get_dof_set_proxy(
            scatra_integrator->nds_growth_deriv_conc())) != ++structure_dofset_counter)
      FOUR_C_THROW("unexpected dof sets in structure field");
    if (structdis->add_dof_set(scatradis->get_dof_set_proxy(
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void SSI::SSICouplingMatchingVolume::set_deriv_simplified_growth_conc_solution(
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void SSI::SSICouplingMatchingVolume::set_deriv_simplified_growth_pot_solution(
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void SSI::SSICouplingMatchingVolumeAndBoundary::set_deriv_simplified_growth_conc_solution(
    Core::FE::Discretization& dis, const Core::LinAlg::Vector<double>& dsimpl_growth_dc,
    unsigned nds)
{
  Core::LinAlg::Vector<double> imported_vec(*dis.dof_col_map(nds), true);
...
/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/
void SSI::SSICouplingMatchingVolumeAndBoundary::set_deriv_simplified_growth_pot_solution(
    Core::FE::Discretization& dis, const Core::LinAlg::Vector<double>& dsimpl_growth_dpot,
    unsigned nds)
{
  Core::LinAlg::Vector<double> imported_vec(*dis.dof_col_map(nds), true);
