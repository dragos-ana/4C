    ...
    InelasticDefgradTransvIsotropElastViscoplastUtils::ErrorType& err_status)
{
  ...
  temp10x1 = determine_local_newton_init_estimate(
      dt, deftensors, last_plastic_strain, err_status, temperature);
  ...
  while (true)
  {
    ...
    manage_evaluation(err_status, deftensors, last_plastic_strain, dt, eval_action, temperature);
    switch (eval_action)
    {
    ...
    else
    {
      ...
      if (local_newton_manager_.is_local_newton_stuck())
      {
        ...
        manage_evaluation(
            err_status, deftensors, last_plastic_strain, dt, eval_action, temperature);
        switch (eval_action)
        {
          case (ViscoplastUtils::EvaluationAction::continue_current_iteration):
          {
    ...
    const double temperature)
{
  ...
  else
  {
    ...
    if (adaptive_estimate_interp_manager_.has_value())
    {
      ...
      Core::LinAlg::Matrix<10, 1> updated_estimate = reestimate_to_restart_local_newton(
          dt, deftensors, last_plastic_strain, eval_action, temperature);
    ...
    const double temperature)
{
  ...
  if (adaptive_estimate_interp_manager_.has_value())
  {
    ...
    // DEBUG
      std::cout << "This has an adaptive estimate_interp_manager? \n";
    ...
    {
      construct_plastic_predictor(dt, deftensors, last_plastic_strain, err_status, temperature);
    ...
    return interpolate_estimate(dt, deftensors, last_plastic_strain, err_status, temperature);
  }
  else
  {
    ...
    const double temperature)
{
  ...
  ViscoplastUtils::StateQuantities state_quantities_elastic_pred =
      evaluate_state_quantities(deftensors.right_cg, temperature,
  ...
  while (true)
  {
    ...
    ViscoplastUtils::StateQuantities interp_state_quantities = evaluate_state_quantities(
        deftensors.right_cg, temperature, interp_inverse_inelastic_defgrad, -1.0, err_status, dt,
        ViscoplastUtils::StateQuantityEvalType::equiv_stress_only);
    if (err_status == InelasticDefgradTransvIsotropElastViscoplastUtils::ErrorType::no_errors)
    {
    ...
    InelasticDefgradTransvIsotropElastViscoplastUtils::ErrorType& err_status) const
{
  ...
  while (true)
  {
    ...
    plastic_strain_rate = viscoplastic_law_->evaluate_plastic_strain_rate(
        integration_input.interp_equiv_stress, plastic_strain, integration_input.dt, err_status);
...
Mat::InelasticDefgradTransvIsotropElastViscoplast::verify_estimate_candidate(const double dt,
    ...
    const double temperature, const Core::LinAlg::Matrix<3, 3>& iFin_candidate,
    const double plastic_strain_candidate)
{
  // initialize output
  ViscoplastUtils::ErrorType err_status{ViscoplastUtils::ErrorType::no_errors};

  // evaluate the candidate state
  state_quantities_ = evaluate_state_quantities(deftensors.right_cg, temperature, iFin_candidate,
  ...
  state_quantity_derivatives_ = evaluate_state_quantity_derivatives(deftensors.right_cg,
      temperature, iFin_candidate, plastic_strain_candidate, err_status, dt,
    ...
    const double temperature)
{
  ...
  while (true)
  {
    ...
    ViscoplastUtils::StateQuantities state_quantities_stress =
        evaluate_state_quantities(deftensors.right_cg, temperature, interp_iFin, 0.0, err_status,
    ...
    err_status =
        verify_estimate_candidate(dt, deftensors, temperature, interp_iFin, interp_plastic_strain);
    if (err_status == ViscoplastUtils::ErrorType::no_errors)
    {
    ...
    const double temperature)
{
  ...
  Core::LinAlg::Matrix<10, 1> updated_estimate =
      interpolate_estimate(dt, deftensors, last_plastic_strain, err_status, temperature);
    ...
    const double temperature)
{
  ...
  ViscoplastUtils::StateQuantities interp_state_quantities =
      evaluate_state_quantities(deftensors.right_cg, temperature, interp_iFin, 0.0, err_status, dt,
          ViscoplastUtils::StateQuantityEvalType::equiv_stress_only);
    ...
    const unsigned int gp)
{
  ...
  ViscoplastUtils::StateQuantities state_quantities_sol =
      evaluate_state_quantities(deftensors.right_cg, time_step_quantities_.current_temperature[gp],
  ...
  ViscoplastUtils::StateQuantities state_quantities_plast_pred =
      evaluate_state_quantities(deftensors.right_cg, time_step_quantities_.current_temperature[gp],
          iFin_plastic_pred, -1.0, err_status, -1.0,
          ViscoplastUtils::StateQuantityEvalType::
              equiv_stress_only);  // we use only dummy values for quantities that are not
                                   // required in this partial computation
