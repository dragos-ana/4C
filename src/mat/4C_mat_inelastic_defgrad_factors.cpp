    ...
    const double last_plastic_strain, ViscoplastUtils::ErrorType& err_status)
{
  ...
  FOUR_C_ASSERT_ALWAYS(adaptive_estimate_interp_manager_.has_value(),
      "This method should not be called if the manager for adaptive estimate interpolation is not "
      "initialized");
    ...
    else
    {
      ...
      switch (parameter()->adaptive_estimate_interp_params().hardening_params.method)
      {
        ...
        default:
        {
          FOUR_C_THROW(
              "{}", get_error_warning_info(std::format(
                        "Unsupported hardening method {} for adaptive estimate interpolation!",
                        parameter()->adaptive_estimate_interp_params().hardening_params.method)));
        }
      }
