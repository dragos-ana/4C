...
Mat::PAR::InelasticDefgradTransvIsotropElastViscoplast::
    ...
    : Parameter(matdata),
      ...
      adaptive_estimate_interpolation_params_(
          matdata.parameters.get<AEINamespace::AEIParams>("ADAPTIVE_ESTIMATE_INTERPOLATION")),
      local_timint_analysis_params_(
          matdata.parameters.get<ViscoplastUtils::LocalTimIntAnalysisParams>(
              "LOCAL_TIMINT_ANALYSIS"))
{
    ...
    : InelasticDefgradFactors(params),
      ...
      adaptive_estimate_interp_manager_(
          parameter()->use_adaptive_estimate_interpolation()
              ? std::make_optional(
                    AEINamespace::AEIManager(parameter()->adaptive_estimate_interpolation_params()))
              : std::nullopt)
{
    ...
    const InelasticDefgradTransvIsotropElastViscoplastUtils::StateQuantityEvalType& eval_type) const
{
  ...
  temp3x3.multiply_nn(1.0, local_integration_input.right_cg, iFinM, 0.0);
  ...
  if (parameter()->mat_behavior() == ViscoplastUtils::MatBehavior::transv_isotropic)
  {
    ...
    {
      FOUR_C_ASSERT_ALWAYS(
          std::abs(local_integration_input.temperature - ref_temperature_) <= 1.0e-16,
  ...
  state_quantities.curr_ST = Mat::ThermalExpansion::compute_thermoelastic_stress_contribution(
      local_integration_input.temperature - ref_temperature_, thermal_expansion_coefficient_,
      potsumel_, gp_, ele_gid_);
  ...
  state_quantities.curr_equiv_plastic_strain_rate =
      viscoplastic_law_->evaluate_plastic_strain_rate(state_quantities.curr_equiv_stress,
          plastic_strain, local_integration_input.step, err_status, update_hist_var_);
  ...
  {
    temp3x3.update(-local_integration_input.step, state_quantities.curr_lpM, 0.0);
    ...
    const bool eval_state) const
{
  ...
  Mat::elast_hyper_get_derivs_of_elastic_right_cg_tensor(Core::LinAlg::make_tensor(iFinM),
      Core::LinAlg::assume_symmetry(Core::LinAlg::make_tensor(local_integration_input.right_cg)),
      dCedC_tensor, dCediFin_tensor);
  ...
  CiFinM.multiply_nn(1.0, local_integration_input.right_cg, iFinM, 0.0);
  ...
  iFinTCM.multiply_tn(1.0, iFinM, local_integration_input.right_cg, 0.0);

  Core::LinAlg::Matrix<3, 3> CeiFinTCM(Core::LinAlg::Initialization::zero);
  temp3x3.multiply_nt(1.0, CeM, iFinM, 0.0);
  CeiFinTCM.multiply_nn(1.0, temp3x3, local_integration_input.right_cg, 0.0);
  ...
  temp3x3.multiply_nn(1.0, local_integration_input.right_cg, iCinM, 0.0);
  ...
  iCM.invert(local_integration_input.right_cg);
  ...
  */
  ...
  InelasticDefgradTransvIsotropElastViscoplastUtils::PlasticStrainRateDerivs evoEqFunctionDers =
      viscoplastic_law_->evaluate_derivatives_of_plastic_strain_rate(
          equiv_stress, plastic_strain, local_integration_input.step, err_status);
  ...
  */
  ...
  if (parameter()->timint_type() == ViscoplastUtils::TimIntType::standard)
  {
    ...
    min_dt_lpM.update(-1.0 * local_integration_input.step, lpM, 0.0);
    ...
    state_quantity_derivatives.curr_dEpdiFin.multiply_nn(
        -local_integration_input.step, expderivV, state_quantity_derivatives.curr_dlpdiFin, 0.0);

    // ... w.r.t. right CG
    state_quantity_derivatives.curr_dEpdC.multiply_nn(
        -local_integration_input.step, expderivV, state_quantity_derivatives.curr_dlpdC, 0.0);

    // ... w.r.t. plastic strain
    state_quantity_derivatives.curr_dEpdepsp.multiply_nn(
        -local_integration_input.step, expderivV, state_quantity_derivatives.curr_dlpdepsp, 0.0);

    // ... w.r.t. temperature
    state_quantity_derivatives.curr_dEpdT.multiply_nn(
        -local_integration_input.step, expderivV, state_quantity_derivatives.curr_dlpdT, 0.0);
  ...
  if (local_timint_analysis_.has_value())
  {
    local_timint_analysis_->increment_constitutive_update_time(gp_,
        get_converged_computation_time("Constitutive update",
            ...
            [this, &iFinM, &reduced_kinematics, &temperature]()
            {
              iFinM =
                  constitutive_update(reduced_kinematics.defgrad, temperature).inv_plastic_defgrad;
            }));
  }
}
    ...
    const Core::LinAlg::Matrix<3, 3>& FredM, const double temperature)
{
  ...
  auto local_integration_input = ViscoplastUtils::LocalIntegrationInput{
      ViscoplastUtils::LocalIntegrationInputConfig{.defgrad = FredM,
          .temperature = temperature,
          .last_inv_inelastic_defgrad = time_step_quantities_.last_plastic_defgrad_inverse[gp_],
          .last_plastic_strain = time_step_quantities_.last_plastic_strain[gp_],
          .step = time_step_tracker_.dt}};
      ...
      if (local_timint_analysis_.has_value())
      {
        local_timint_analysis_->increment_aei_starting_point_determination_time(
            gp, get_converged_computation_time("AEI Starting point determination",
                    parameter()->local_timint_analysis_params().rel_tol_computation_time,
                    [this, gp]() { update_aei_starting_point(gp); }));

        local_timint_analysis_->set_aei_interp_point_equiv_stress_history(gp,
            adaptive_estimate_interp_manager_.value(), get_input_equiv_stress_starting_point(gp));
      }
    }
    ...
    InelasticDefgradTransvIsotropElastViscoplastUtils::ErrorType& err_status)
{
  ...
  if (parameter()->timint_type() == ViscoplastUtils::TimIntType::standard)
  {
    ...
    temp3x3.multiply_nn(1.0, local_integration_input.elastic_predictor_inverse_plastic_defgrad,
        state_quantities_.curr_EpM, 0.0);
    resFM.update(1.0, iFinM, -1.0, temp3x3, 0.0);

    // calculate residual of the equation for plastic strain
    resepsp = plastic_strain - local_integration_input.last_plastic_strain -
              local_integration_input.step * state_quantities_.curr_equiv_plastic_strain_rate;
  }
  ...
  else if (parameter()->timint_type() == ViscoplastUtils::TimIntType::logarithmic)
  {
    ...
    last_FinM.invert(local_integration_input.elastic_predictor_inverse_plastic_defgrad);
    ...
    // calculate residual of the equation for inelastic defgrad
    resFM.update(1.0, logT, local_integration_input.step, state_quantities_.curr_lpM, 0.0);
    ...
    InelasticDefgradTransvIsotropElastViscoplastUtils::ErrorType& err_status)
{
  ...
  if (parameter()->timint_type() == ViscoplastUtils::TimIntType::standard)
  {
    ...
    Core::LinAlg::FourTensorOperations::multiply_matrix_four_tensor<3>(tempFourTensor,
        local_integration_input.elastic_predictor_inverse_plastic_defgrad, dEpdiFin_FourTensor,
        true);
    ...
    temp3x3.multiply_nn(
        -1.0, local_integration_input.elastic_predictor_inverse_plastic_defgrad, dEpdepsp_M, 0.0);
    ...
    J_epsp_iFin.update(
        -local_integration_input.step * state_quantity_derivatives_.curr_dpsr_dequiv_stress,
    ...
    J_epsp_epsp = 1.0 - local_integration_input.step * state_quantity_derivatives_.curr_dpsr_depsp;
  }
  else if (parameter()->timint_type() == ViscoplastUtils::TimIntType::logarithmic)
  // logarithmic time integration
  {
    ...
    J_iFin_iFin.update(1.0, dlogTdiFin, local_integration_input.step,
        state_quantity_derivatives_.curr_dlpdiFin, 0.0);

    // compute 9x1 north-east component block of the Jacobian (derivative of residual for
    // inelastic deformation gradient w.r.t. plastic strain)
    J_iFin_epsp.update(
        local_integration_input.step, state_quantity_derivatives_.curr_dlpdepsp, 0.0);
    ...
    ViscoplastUtils::ErrorType& err_status)
{
  ...
  if (parameter()->use_local_substepping())
  {
    while (!local_substepping_utils_.end_substepping())
    {
      ...
      auto curr_local_integration_input = ViscoplastUtils::LocalIntegrationInput{
          ViscoplastUtils::LocalIntegrationInputConfig{.defgrad = curr_FM,
      ...
      local_timint_analysis_->append_lnl_tracking_data(gp_,
          {.local_iter = local_newton_manager_.iter(),
              ...
              .plastic_strain_increment =
                  state_quantities_.curr_equiv_plastic_strain_rate * local_integration_input.step});
    }
      ...
      if (local_timint_analysis_.has_value() && local_timint_analysis_->track_quantities())
      {
        local_timint_analysis_->append_lnl_tracking_data(
            gp_, {.local_iter = local_newton_manager_.iter(),
                     ...
                     .equiv_stress = state_quantities_.curr_equiv_stress,
                     .plastic_strain = local_newton_manager_.sol()(9),
                     .plastic_strain_increment = state_quantities_.curr_equiv_plastic_strain_rate *
                                                 local_integration_input.step});
      }
    ...
    ViscoplastUtils::ErrorType& err_status)
{
  ...
  adaptive_estimate_interp_manager_->set_current_interp_point(
      gp_, AEINamespace::CurrentInterpPointPreset::plastic_predictor);
    ...
    if (err_status == InelasticDefgradTransvIsotropElastViscoplastUtils::ErrorType::no_errors)
    {
                   ...
                   .plastic_predictor_construction.relative_understress_tol)
      {
        ...
        adaptive_estimate_interp_manager_->adapt_interpolation_interval(
            gp_, AEINamespace::InterpolationIntervalShift::towards_elastic_pred);
        adaptive_estimate_interp_manager_->set_current_interp_point(
            gp_, AEINamespace::CurrentInterpPointPreset::plastic_pred_construct_update);
      }
      else
      {
        // we shift towards the preliminary plastic predictor
        adaptive_estimate_interp_manager_->adapt_interpolation_interval(
            gp_, AEINamespace::InterpolationIntervalShift::towards_plastic_pred);
        adaptive_estimate_interp_manager_->set_current_interp_point(
            gp_, AEINamespace::CurrentInterpPointPreset::plastic_pred_construct_update);
      }
    }
...
double Mat::InelasticDefgradTransvIsotropElastViscoplast::integrate_plastic_strain(
    const AEINamespace::InputHardeningIntegration& integration_input,
    InelasticDefgradTransvIsotropElastViscoplastUtils::ErrorType& err_status) const
{
  ...
  const AEINamespace::HardeningParams& hardening_params =
      parameter()->adaptive_estimate_interpolation_params().hardening;

  FOUR_C_ASSERT(hardening_params.method == AEINamespace::HardeningMethod::integrate_via_evol_eqs,
      "{}",
      get_error_warning_info(
    ...
    InelasticDefgradTransvIsotropElastViscoplastUtils::ErrorType& err_status)
{
  ...
  while (true)
  {
    ...
    if (err_status == ViscoplastUtils::ErrorType::no_errors)
    {
      ...
      if (!exhibits_plastic_flow(state_quantities_stress_only.curr_equiv_stress,
              local_integration_input.last_plastic_strain))
      {
        adaptive_estimate_interp_manager_->adapt_interpolation_interval(
            gp_, AEINamespace::InterpolationIntervalShift::towards_elastic_pred);
        adaptive_estimate_interp_manager_->set_current_interp_point(
            gp_, AEINamespace::CurrentInterpPointPreset::estimate_interpolation_update);
        continue;
      }
    }
    else
    ...
    {
      case AEINamespace::HardeningMethod::use_previous:
      {
        interp_plastic_strain = local_integration_input.last_plastic_strain;
        break;
      }
      case AEINamespace::HardeningMethod::integrate_via_evol_eqs:
      {
        interp_plastic_strain = integrate_plastic_strain(
            AEINamespace::InputHardeningIntegration{
                .interp_equiv_stress = state_quantities_stress_only.curr_equiv_stress,
                .last_plastic_strain = local_integration_input.last_plastic_strain,
                .step = local_integration_input.step},
            err_status);
    ...
    ViscoplastUtils::EvaluationAction& eval_action)
{
  ...
  adaptive_estimate_interp_manager_->set_current_interp_point(
      gp_, AEINamespace::CurrentInterpPointPreset::intermediate_point);
  ...
  if (!exhibits_plastic_flow(
          interp_state_quantities.curr_equiv_stress, local_integration_input.last_plastic_strain))
  {
    adaptive_estimate_interp_manager_->adapt_interpolation_interval(
        gp_, AEINamespace::InterpolationIntervalShift::towards_elastic_pred);
    return reinterpolate_with_updated_bounds(local_integration_input, eval_action);
    ...
    const unsigned int gp)
{
  ...
  const AEINamespace::AEIParams& aei_params = parameter()->adaptive_estimate_interpolation_params();

  // starting points are set based on the specified method
  switch (aei_params.estimate_interpolation.starting_point_type)
  {
    case AEINamespace::StartingPointType::user_set:
    {
      adaptive_estimate_interp_manager_->set_user_starting_point(gp);
      break;
    }
    case AEINamespace::StartingPointType::equiv_stress_history:
    {
...
/*--------------------------------------------------------------------*
 *--------------------------------------------------------------------*/
AEINamespace::InputEquivStressStartingPoint
    ...
    const unsigned int gp)
{
  ...
  auto local_integration_input =
      ViscoplastUtils::LocalIntegrationInput{ViscoplastUtils::LocalIntegrationInputConfig{
          .defgrad = time_step_quantities_.current_defgrad[gp],
...
Mat::InelasticDefgradTransvIsotropElastViscoplast::save_overall_state() const
{
  return {.time_step_quantities = time_step_quantities_,
      ...
      .aei_manager_state = adaptive_estimate_interp_manager_->get_state()};
}
    ...
    const InelasticDefgradTransvIsotropElastViscoplast::OverallState& overall_state)
{
  ...
  adaptive_estimate_interp_manager_->reinstate_state(overall_state.aei_manager_state);
}
...
void Mat::InelasticDefgradTransvIsotropElastViscoplast::init_local_timint_analysis()
{
  ...
  if (static_cast<int>(ele_gid_) != parameter()->local_timint_analysis_params().ele_gid) return;
  ...
  if (!local_timint_analysis_.has_value())
  {
    local_timint_analysis_.emplace(parameter()->use_adaptive_estimate_interpolation(), num_gp_);
  }
}
