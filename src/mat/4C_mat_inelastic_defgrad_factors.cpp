...
namespace
{
      ...
      const LocalNewtonGuessInterpolation::PlasticPredictorRotationType plast_pred_rot_type)
  {
    ...
    switch (plast_pred_elastic_stretch_eigenval_type)
    {
      ...
      {
        for (int i = 0; i < 3; ++i)
          eigenvalue_matrix(i, i) = 1.0 / last_FeM_matstretch_inverse_eigenval[i];
        break;
      }
    ...
    const Core::LinAlg::Matrix<3, 3>& defgrad)
{
  ...
  if (parameter()->use_lngi())
  {
    ...
    */
    ...
    {
      // I removed these in a previous commit ...
    }
    ...
    const Core::LinAlg::Matrix<3, 3>& FM)
{
         ...
         err_status != ErrorType::no_errors)
  {
    ...
    // DEBUG
    std::cout << "lngi_step_counter: " << lngi_step_counter << std::endl;
    ...
    {  // if the interpolation factors are near 0, then we explicitly use the elastic predictor
       // based on the previous timestep; otherwise we will get mismatches due to the
       // interpolation of the elastic predictor which, even though they are within machine
       // precision, may lead to strange effects (such as in stress relaxation: stress of the
       // elastic predictor is >= yield stress, but after interpolation stress < yield stress,
       // which halts the LNGI completely)
    ...
    Core::LinAlg::Matrix<3, 3>& curr_CM)
{
  ...
  if (parameter()->use_lngi())
  {
    ...
    // compute equivalent stresses associated with the lower bound and with the interpolation
    // point to be evaluated next interpolate inverse plastic deformation gradient
        ...
        lbound_equiv_stress_eval_err_status == ErrorType::no_errors)
    {
      ...
      // next point should be evaluated next as a candidate for an initial guess (relative
      // deviation must be higher than a specified value)
    ...
    const double optimal_xi_lambda_2, const std::array<double, 3> optimal_xi_eigenvect_rot)
{
  ...
  // save "last" stretch and rotation components
  time_step_quantities_.last_elastic_defgrad_material_stretch_inverse_[gp].invert(
      last_elastic_defgrad_material_stretch);
  time_step_quantities_.last_inverse_elastic_stretch_eigenval_[gp] = {
      1.0 / last_elastic_spectral_pairs[0].first, 1.0 / last_elastic_spectral_pairs[1].first,
      1.0 / last_elastic_spectral_pairs[2].first};
}
