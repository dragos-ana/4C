    ...
    Core::LinAlg::Matrix<3, 3>& iFinM)
{
  ...
  if (debug_output_ele_gp(debug_ele_gid_vec, debug_gp_vec, ele_gid_, gp_))
  {
    ...
    std::cout << pred_adapt_utils_.last_xi_[gp_] << std::endl;
    std::cout << "last_max_xi: " << std::endl;
    std::cout << pred_adapt_utils_.last_max_xi_[gp_] << std::endl;
    std::cout << "optimal_xi: " << std::endl;
    std::cout << pred_adapt_utils_.optimal_xi_[gp_] << std::endl;
    std::cout << "current_xi: " << std::endl;
    std::cout << pred_adapt_utils_.current_xi_[gp_] << std::endl;
    std::cout << "current_max_xi: " << std::endl;
    std::cout << pred_adapt_utils_.current_max_xi_[gp_] << std::endl;
  }
#endif
...
void Mat::InelasticDefgradTransvIsotropElastViscoplast::update()
{
  ...
  {
    std::cout << "update: current_xi[" << 0 << "]: " << pred_adapt_utils_.current_xi_[0]
              << std::endl;
    std::cout << "update: current_max_xi[" << 0 << "]: " << pred_adapt_utils_.current_max_xi_[0]
              << std::endl;
  }
#endif
    ...
    const bool check_original_pred)
{
  ...
  if (debug_output_ele_gp(debug_ele_gid_vec, debug_gp_vec, ele_gid_, gp_))
  {
    ...
    std::cout << std::string(5, '.')
              << "starting point: xi = " << pred_adapt_utils_.current_xi_[gp_] << std::endl;
  }
#endif
         ...
         err_status != ErrorType::no_errors)
  {
    ...
    if (err_status != ErrorType::no_errors)
    {
      ...
      // \f$
      {
        ...
        if (debug_output_ele_gp(debug_ele_gid_vec, debug_gp_vec, ele_gid_, gp_))
        {
          std::cout << "Adapted xi_u_ to " << pred_adapt_utils_.xi_u_ << std::endl;
        }
#endif
      }
      else  // there is "too much" plastic strain rate -> leads to overflow error
      {
        ...
        if (debug_output_ele_gp(debug_ele_gid_vec, debug_gp_vec, ele_gid_, gp_))
        {
          std::cout << "Adapted xi_l_ to " << pred_adapt_utils_.xi_l_ << std::endl;
        }
#endif
      }
    }
  ...
  if (debug_output_ele_gp(debug_ele_gid_vec, debug_gp_vec, ele_gid_, gp_))
  {
    std::cout << "current_xi_[" << gp_ << "]: " << pred_adapt_utils_.current_xi_[gp_] << std::endl;
    std::cout << std::string(40, '-') << std::endl;
  }
    ...
    Core::LinAlg::Matrix<3, 3>& curr_CM)
{
  ...
  if (debug_output_ele_gp(debug_ele_gid_vec, debug_gp_vec, ele_gid_, gp_))
  {
    std::cout << "err_status: " << magic_enum::enum_name(err_status) << std::endl;
  }
    ...
    const int gp, const double newton_starting_point)
{
  ...
  // interpolation factor. All other components consistently follow the same interpolation factor
  // (1D plastic incompressibility).
