...
FOUR_C_NAMESPACE_OPEN namespace
{
  ...
  retrieve_error_registration_settings(const Core::Mat::PAR::Parameter::Data& matdata)
  {
    return ViscoplastUtils::ErrorRegistrationSettings{
        ...
        .register_plastic_strain_incr_derivs_overflow =
            matdata.parameters.group("ERROR_REGISTRATION_SETTINGS")
                .get<bool>("REGISTER_PLASTIC_STRAIN_INCR_DERIVS_OVERFLOW"),
  ...
  Mat::InelasticDefgradTransvIsotropElastViscoplastUtils::LocalTimIntAnalysisParams
  retrieve_local_timint_analysis_params(const Core::Mat::PAR::Parameter::Data& matdata)
  {
    const bool analyze_local_timint =
        matdata.parameters.group("LOCAL_TIMINT_ANALYSIS").get<bool>("ANALYZE_LOCAL_TIMINT");
    // if analysis framework is used, get the associated parameters; else, use dummy values
    if (analyze_local_timint)
    {
      return {
          .ele_gid = static_cast<unsigned int>(
              matdata.parameters.group("LOCAL_TIMINT_ANALYSIS").get<int>("ELE_GID")),
          .rel_tol_computation_time = matdata.parameters.group("LOCAL_TIMINT_ANALYSIS")
              .get<double>("REL_TOL_COMPUTATION_TIME"),
      };
    }
    else
    {
      return {.ele_gid = 0, .rel_tol_computation_time = 0.0};
    }
  }
...
Mat::PAR::InelasticDefgradTransvIsotropElastViscoplast::
    ...
    : Parameter(matdata),
      ...
      analyze_local_timint_(
          matdata.parameters.group("LOCAL_TIMINT_ANALYSIS").get<bool>("ANALYZE_LOCAL_TIMINT")),
      local_timint_analysis_params_(retrieve_local_timint_analysis_params(matdata))
{
...
Mat::InelasticDefgradTransvIsotropElastViscoplast::InelasticDefgradTransvIsotropElastViscoplast(
    ...
    : InelasticDefgradFactors(params),
      ...
      adaptive_estimate_interp_manager_(
          parameter()->use_adaptive_estimate_interp()
              ? std::make_optional(ViscoplastUtils::AdaptiveEstimateInterpolationManager(
                    parameter()->adaptive_estimate_interp_params()))
              : std::nullopt)
{
  ...
  // initialize plastic flow tracking vector for a single Gauss point; we resize it afterwards
    ...
    const int eleGID)
{
  ...
  // local time integration analysis: initialization, only in the corresponding element and on the
  // owner rank!
  if (parameter()->analyze_local_timint()) init_local_timint_analysis();

  // set total time for the local time integration analysis -> only do it once, at the first GP
  if (local_timint_analysis_.has_value() && gp == 0)
  {
    local_timint_analysis_->set_time(*context.total_time);
...
void Mat::InelasticDefgradTransvIsotropElastViscoplast::prepare_constitutive_update()
{
  ...
  // We increment timestep or global iteration of the analysis framework in the return mapping
  // preparation, not in the pre-evaluation routine of the material, to circumvent the double
  // material evaluation calls in issue #121; also, we set the used starting point
  if (local_timint_analysis_.has_value() && local_timint_analysis_->track_quantities())
  {
    // incrementation done only once for the first Gauss point
    if (gp_ == 0) local_timint_analysis_->increment_timestep_or_global_iter();
    if (adaptive_estimate_interp_manager_.has_value())
      local_timint_analysis_->set_aei_starting_point(
          gp_, adaptive_estimate_interp_manager_->starting_point(gp_));
  }
}
    ...
    const Core::LinAlg::Matrix<6, 9>& dSdiFinj, Core::LinAlg::Matrix<6, 6>& cmatadd)
{
  ...
  // standard time integration
  {
    ...
    Core::LinAlg::FourTensorOperations::multiply_matrix_four_tensor<3>(
        tempFourTensor, time_step_quantities_.last_plastic_defgrad_inverse[gp_], dEpdC_FourTensor);
    ...
    Core::LinAlg::Matrix<3, 3>& iFinM)
{
  ...
  // constitutive update under the same conditions, and not simply return the
  // computed value)
  ...
  // via constitutive update (elastic predictor -- viscoplastic corrector).

  iFinM = constitutive_update(FredM).inv_plastic_defgrad;

  // track return mapping time
  if (local_timint_analysis_.has_value())
  {
    local_timint_analysis_->increment_constitutive_update_time(gp_,
        get_converged_computation_time("Constitutive update",
            parameter()->local_timint_analysis_params().rel_tol_computation_time,
            [this, &iFinM, &FredM]() { iFinM = constitutive_update(FredM).inv_plastic_defgrad; }));
  }
}
    ...
    const Core::LinAlg::Matrix<3, 3>& FredM)
{
  ...
  bool pred_is_sol =
      check_elastic_predictor(deftensors.right_cg, iFinM_pred, plastic_strain_pred, err_status);
  ...
  else  // predictor does not suffice
  {
    ...
    Core::LinAlg::Matrix<10, 1> sol = viscoplastic_correction(deftensors, err_status);
    ...
    if (err_status != ViscoplastUtils::ErrorType::no_errors)
    {
      ...
      FOUR_C_THROW(
          "{}", get_error_warning_info(std::format(
                    "Viscoplastic correction was not successful! Error status: {}", err_status)));
    }
...
void Mat::InelasticDefgradTransvIsotropElastViscoplast::update()
{
  for (unsigned int gp = 0; gp < num_gp_; ++gp)
  {
    ...
    {
      update_adaptive_estimate_interp_starting_points(gp);

      if (local_timint_analysis_.has_value())
      {
        local_timint_analysis_->increment_aei_starting_point_determination_time(
            gp, get_converged_computation_time("AEI Starting point determination",
                    parameter()->local_timint_analysis_params().rel_tol_computation_time,
                    [this, gp]() { update_adaptive_estimate_interp_starting_points(gp); }));

        local_timint_analysis_->set_aei_interp_point_optimal_equiv_stress(
            gp, adaptive_estimate_interp_manager_.value(), get_input_optimal_equiv_stress(gp));
      }
    }

    if (local_timint_analysis_.has_value())
    {
      // the order of operations is important for the analysis framework of local time
      // integration: we need to first write out all Gauss point tables, than write the overall
      // table, and only then reset each Gauss point individually
      local_timint_analysis_->update_gp_totals_and_write_gp_tables(gp);
    }
  ...
  if (local_timint_analysis_.has_value())
  {
    local_timint_analysis_->write_overall_timestep_table_to_csv();
    for (unsigned int gp = 0; gp < num_gp_; ++gp) local_timint_analysis_->reset(gp);
  };
}
    ...
    ViscoplastUtils::ErrorType& err_status)
{
  ...
  if (parameter()->use_local_substepping())
  {
    while (!local_substepping_utils_.end_substepping())
    {
      ...
      err_status = ViscoplastUtils::ErrorType::no_errors;
      sol =
          local_newton_loop(curr_deftensors, time_step_quantities_.last_substep_plastic_strain[gp_],
      ...
      if (local_timint_analysis_.has_value() && local_timint_analysis_->track_quantities())
      {
        local_timint_analysis_->increment_num_lnl_iters(gp_, local_newton_manager_.iter());
      }
  ...
  else
  {
    ...
    sol = local_newton_loop(deftensors, time_step_quantities_.last_plastic_strain[gp_],
    ...
    if (err_status != ViscoplastUtils::ErrorType::no_errors)
    {
      FOUR_C_THROW(
          "{}", get_error_warning_info(std::format(
                    "There was an error within the local Newton! Error status: {}", err_status)));
    }
    ...
    InelasticDefgradTransvIsotropElastViscoplastUtils::ErrorType& err_status)
{
  ...
  if (err_status != ViscoplastUtils::ErrorType::no_errors)
  {
    FOUR_C_THROW("{}",
        get_error_warning_info(
            std::format("Local Newton cannot even be initialized due to error: {}", err_status)));
  }
  ...
  while (true)
  {
    ...
    // evaluate residual
    residual = evaluate_local_newton_residual(deftensors.right_cg, local_newton_manager_.sol(),
    ...
    InelasticDefgradTransvIsotropElastViscoplastUtils::ErrorType& err_status)
{
  switch (local_newton_manager_.params().diver_cont)
  {
    case FourC::Mat::InelasticDefgradTransvIsotropElastViscoplastUtils::LocalNewtonDiverCont::stop:
    {
      ...
      FOUR_C_THROW("{}",
          get_error_warning_info(
              std::format("There is no Local Newton convergence! Error status: {}", err_status)));
      return;
    }
            ...
            if (!residual_within_bounds)
            {
              FOUR_C_THROW("{}",
                  get_error_warning_info(std::format(
                      ...
                      local_newton_manager_.convergence_quantities().residual_norm,
                      local_newton_manager_.params().res_tol,
                      local_newton_manager_.params().max_exceedance_fact_res_tol, err_status)));
            }
            ...
            if (!incr_ratio_within_bounds)
            {
              FOUR_C_THROW("{}",
                  get_error_warning_info(std::format(
                      ...
                      local_newton_manager_.convergence_quantities().increment_norm,
                      local_newton_manager_.params().incr_tol,
                      local_newton_manager_.params().max_exceedance_fact_incr_tol, err_status)));
            }
          ...
          default:
            FOUR_C_THROW(
                "{}", get_error_warning_info(std::format(
                          ...
                          local_newton_manager_.params().conv_check)));
        }
      }
    ...
    const double plastic_strain_pred, ViscoplastUtils::ErrorType& err_status)
{
  ...
  // verify obtained stress state and error status
  if (err_status != ViscoplastUtils::ErrorType::no_errors)
  {
    // set equivalent stress of the elastic predictor
    if (std::isnan(state_quantities_.curr_equiv_stress))
    {
      equiv_stress_elastic_predictor_ = std::numeric_limits<double>::max();
    }
    else
    {
      equiv_stress_elastic_predictor_ = state_quantities_.curr_equiv_stress;
    }
    ...
    const Core::LinAlg::Matrix<6, 9>& dSdiFinj)
{
  ...
  for (int i = 0; i < static_cast<int>(indices_array.size()); i++)
  {
    ...
    perturbed_iFinM = constitutive_update(perturbed_FM).inv_plastic_defgrad;
  ...
  else
  {
    ...
    if (adaptive_estimate_interp_manager_.has_value())
    {
      ...
      Core::LinAlg::Matrix<10, 1> updated_estimate =
          reestimate_to_restart_local_newton(dt, deftensors, last_plastic_strain, eval_action);


      if (local_timint_analysis_.has_value() && adaptive_estimate_interp_manager_.has_value() &&
          local_timint_analysis_->track_quantities())
        local_timint_analysis_->increment_aei_num_reestimations(
            gp_, adaptive_estimate_interp_manager_->num_reestimations());

      if (eval_action == ViscoplastUtils::EvaluationAction::exit_with_error &&
          !parameter()->use_local_substepping())
      {
        // throw error
        FOUR_C_THROW(
            "{}", get_error_warning_info(std::format(
      ...
      // compute increment mapping the current solution vector to the updated estimate
    ...
    InelasticDefgradTransvIsotropElastViscoplastUtils::ErrorType& err_status)
{
  ...
  if (adaptive_estimate_interp_manager_.has_value())
  {
    ...
    adaptive_estimate_interp_manager_->reset_and_construct_prelim_plastic_pred(gp_, deftensors);

    // construct the plastic predictor such that its stress state lies on the yield surface
    // (only for yield-surface-based viscoplasticity formulations)
    if (viscoplastic_law_->uses_yield_surface())
    {
      construct_plastic_predictor(dt, deftensors, last_plastic_strain, err_status);

      if (local_timint_analysis_.has_value() && adaptive_estimate_interp_manager_.has_value() &&
          local_timint_analysis_->track_quantities())
        local_timint_analysis_->increment_aei_num_plastic_pred_construct_iters(
            gp_, adaptive_estimate_interp_manager_->num_plastic_pred_construct_iters());
    ...
    // interpolate initial estimate for the local Newton loop
    return interpolate_estimate(dt, deftensors, last_plastic_strain, err_status);
  }
  else
  {
...
/*--------------------------------------------------------------------*
 *--------------------------------------------------------------------*/
void Mat::InelasticDefgradTransvIsotropElastViscoplast::construct_plastic_predictor(const double dt,
    const InelasticDefgradTransvIsotropElastViscoplastUtils::LocalIntegrationDeformationTensors&
        deftensors,
    const double last_plastic_strain, ViscoplastUtils::ErrorType& err_status)
{
  ensure_error_free_evaluation(err_status);
  if (!adaptive_estimate_interp_manager_.has_value())
  {
  ...
  // evaluate the relative yield stress deviation associated with the elastic predictor
  double rel_stress_deviation_elastic_pred;
  ViscoplastUtils::StateQuantities state_quantities_elastic_pred =
      evaluate_state_quantities(deftensors.right_cg,
          deftensors.elastic_predictor_inverse_plastic_defgrad, last_plastic_strain, err_status, dt,
  ...
  if (err_status != InelasticDefgradTransvIsotropElastViscoplastUtils::ErrorType::no_errors)
  {
    ...
    rel_stress_deviation_elastic_pred =
        2.0 * parameter()->adaptive_estimate_interp_params().relative_understress_tol;
  }
  else
  {
    rel_stress_deviation_elastic_pred =
        viscoplastic_law_->evaluate_stress_ratio(
            state_quantities_elastic_pred.curr_equiv_stress, last_plastic_strain) -
        1.0;
    if (rel_stress_deviation_elastic_pred < 0.0)
    {
      FOUR_C_THROW("{}", get_error_warning_info(std::format(
                             "Your elastic predictor is under the yield surface, relative yield "
                             "stress deviation:  "
  ...
  // evaluate the relative yield stress deviation associated with the plastic predictor
  double rel_stress_deviation_plastic_pred;
  err_status = InelasticDefgradTransvIsotropElastViscoplastUtils::ErrorType::no_errors;
  ViscoplastUtils::StateQuantities state_quantities_plastic_pred =
      evaluate_state_quantities(deftensors.right_cg,
          adaptive_estimate_interp_manager_->get_inverse_inelastic_defgrad_plastic_pred(
              gp_, deftensors.inv_defgrad),
          last_plastic_strain, err_status, dt,
          ViscoplastUtils::StateQuantityEvalType::equiv_stress_only);
  if (err_status != ViscoplastUtils::ErrorType::no_errors)
  {
    FOUR_C_THROW("{}", get_error_warning_info("Inconsistent preliminary plastic predictor stress! "
  ...
  // that the preliminary plastic predictor with scale_unit eigenvalues must have 0.0 equivalent
  // stress!
  if (parameter()->mat_behavior() == ViscoplastUtils::MatBehavior::isotropic &&
      parameter()->adaptive_estimate_interp_params().plastic_pred_elastic_stretch_eigenval_type ==
          ViscoplastUtils::PlasticPredictorElasticStretchEigenvalType::scale_unit)
  {
    if (state_quantities_plastic_pred.curr_equiv_stress > 1.0e-8)
    {
      FOUR_C_THROW(
          "{}", get_error_warning_info(std::format(
  ...
  // preliminary verification: if the stress state of the elastic predictor is already within
  // the set yield stress tolerance, we can directly exit and use the elastic predictor as our
  // initial estimate; the most reliable approach for the subsequent re-estimation approach is
  // to keep the preliminary plastic predictor as the plastic predictor, because we know that it
  // is under the yield surface in contrast to the elastic predictor!
  if (rel_stress_deviation_elastic_pred <=
      parameter()->adaptive_estimate_interp_params().relative_understress_tol)
  {
  ...
  else if (rel_stress_deviation_plastic_pred <= 0.0)
  {
    ...
    while (true)
    {
      ...
      if (!adaptive_estimate_interp_manager_->is_plastic_pred_construct_possible(gp_))
      {
        // throw error
        FOUR_C_THROW("{}", get_error_warning_info(std::format(
                               "Plastic predictor construction has failed after {} iterations",
                               parameter()
                                   ->adaptive_estimate_interp_params()
                                   .max_num_plastic_pred_construct_iters)));
      }
      ...
      ViscoplastUtils::StateQuantities interp_state_quantities =
          evaluate_state_quantities(deftensors.right_cg, interp_inverse_inelastic_defgrad, -1.0,
              err_status, dt, ViscoplastUtils::StateQuantityEvalType::equiv_stress_only);
      if (err_status == InelasticDefgradTransvIsotropElastViscoplastUtils::ErrorType::no_errors)
      {
        // compute and verify relative yield stress deviation
        const double rel_yield_stress_deviation_interp =
            viscoplastic_law_->evaluate_stress_ratio(
                interp_state_quantities.curr_equiv_stress, last_plastic_strain) -
            1.0;
        // we only break out if we are under the yield surface and the stress state is within its
        ...
        {
          // we set an "under_yield_surface" error to make sure that the construction interval
          // is shifted towards the elastic predictor
          err_status = ViscoplastUtils::ErrorType::under_yield_surface;
          adaptive_estimate_interp_manager_->adapt_interpolation_interval_and_point(
              gp_, err_status);
        }
        else
        {
      ...
      else
      {
        ...
        adaptive_estimate_interp_manager_->adapt_interpolation_interval_and_point(gp_, err_status);
      }
    }
  }
}
    ...
    InelasticDefgradTransvIsotropElastViscoplastUtils::ErrorType& err_status) const
{
  ...
  const ViscoplastUtils::AdaptiveEstimateInterpolationHardeningParams& hardening_params =
      parameter()->adaptive_estimate_interp_params().hardening_params;


  if (hardening_params.method !=
    ...
    FOUR_C_THROW("{}",
        get_error_warning_info(
            std::format("This method should only be called if the hardening integration method is "
                ...
                parameter()->adaptive_estimate_interp_params().hardening_params.method)));
  }
  ...
  // initialize quantities involved in the hardening integration
  ...
  while (true)
  {
    ...
    if (iter > parameter()->adaptive_estimate_interp_params().hardening_params.max_iter_integration)
    {
      if (!hardening_params.allow_integration_failure)
      {
        FOUR_C_THROW("{}", get_error_warning_info("Could not integrate plastic strain!"));
      }
      else
      {
        if (show_warnings(ele_gid_))
        {
          std::cout << get_error_warning_info("WARNING: could not integrate plastic strain");
        }
        // decide whether the state is shifted too much towards the elastic or the plastic
        // predictor, and mark it accordingly
        const double rel_yield_stress_deviation =
            viscoplastic_law_->evaluate_stress_ratio(
                integration_input.interp_equiv_stress, integration_input.last_plastic_strain) -
            1.0;
        if (rel_yield_stress_deviation <= 0.0)
        {
          FOUR_C_THROW("{}",
              get_error_warning_info(std::format(
                  "This method should not have been called in the first place! The relative yield "
                  "stress "
                  "deviation is {} for the input stress {}",
                  rel_yield_stress_deviation, integration_input.interp_equiv_stress)));
        }

        if (rel_yield_stress_deviation <= hardening_params.relative_overstress_tol)
        {
        ...
        {
          // mark artificially with "overflow_error" error -> this will lead to a shift
          // towards the plastic predictor in the next estimate interpolation iteration
          err_status = ViscoplastUtils::ErrorType::overflow_error;
          return -1;
        }
      }
    ...
    plastic_strain_rate = viscoplastic_law_->evaluate_plastic_strain_rate(
        integration_input.interp_equiv_stress, plastic_strain, integration_input.dt,
        parameter()->error_registration_settings(), err_status);
    ...
    residual = plastic_strain - integration_input.last_plastic_strain -
               integration_input.dt * plastic_strain_rate;
    ...
    // compute jacobian
    jacobian = 1.0 - integration_input.dt * deriv_plastic_strain_rate;
...
Mat::InelasticDefgradTransvIsotropElastViscoplast::verify_estimate_candidate(const double dt,
    ...
    const Core::LinAlg::Matrix<3, 3>& iFin_candidate, const double plastic_strain_candidate)
{
  ...
  state_quantities_ = evaluate_state_quantities(deftensors.right_cg, iFin_candidate,
      plastic_strain_candidate, err_status, dt, ViscoplastUtils::StateQuantityEvalType::full_eval);

  // evaluation check
  if (err_status != ViscoplastUtils::ErrorType::no_errors) return err_status;


  // verify whether the stress state lies under the yield surface
  const bool is_under_yield_surface =
      (viscoplastic_law_->evaluate_stress_ratio(
           state_quantities_.curr_equiv_stress, plastic_strain_candidate) < 1.0);
  if (is_under_yield_surface) return ViscoplastUtils::ErrorType::under_yield_surface;
  if (err_status != ViscoplastUtils::ErrorType::no_errors) return err_status;

  // evaluate the candidate state linearization
  state_quantity_derivatives_ = evaluate_state_quantity_derivatives(deftensors.right_cg,
      iFin_candidate, plastic_strain_candidate, err_status, dt,
    ...
    const double last_plastic_strain, ViscoplastUtils::ErrorType& err_status)
{
  ...
  // declare interpolated estimate for the inverse plastic defgrad and
  // the plastic strain
  ...
  while (true)
  {
    ...
    // interpolate inverse plastic deformation gradient with the current interpolation points
    // saved within the dedicated manager
    if (adaptive_estimate_interp_manager_->current_interp_point(gp_) == 0.0)
    {
      interp_iFin =
          deftensors
              .elastic_predictor_inverse_plastic_defgrad;  // this is necessary, since interpolating
                                                           // with preconditioned matrices can lead
                                                           // to (slight) deviations from the
                                                           // elastic predictor
    }
    else
    {
    ...
    ViscoplastUtils::StateQuantities state_quantities_stress =
        evaluate_state_quantities(deftensors.right_cg, interp_iFin, 0.0, err_status,
            time_step_tracker_.dt, ViscoplastUtils::StateQuantityEvalType::equiv_stress_only);

    if (err_status == ViscoplastUtils::ErrorType::no_errors)
    {
      // detect "under the yield surface" error state
      if (viscoplastic_law_->evaluate_stress_ratio(
              state_quantities_stress.curr_equiv_stress, last_plastic_strain) < 1.0)
      {
        err_status = ViscoplastUtils::ErrorType::under_yield_surface;
      }

      // detect increased stress state with respect to the elastic predictor
      if (parameter()->adaptive_estimate_interp_params().bound_stress_by_elastic_predictor &&
          state_quantities_stress.curr_equiv_stress >= equiv_stress_elastic_predictor_)
        err_status = ViscoplastUtils::ErrorType::overflow_error;
    }
    ...
    switch (parameter()->adaptive_estimate_interp_params().hardening_params.method)
    {
      case ViscoplastUtils::AdaptiveEstimateInterpolationHardeningMethod::use_previous:
      {
      ...
      case ViscoplastUtils::AdaptiveEstimateInterpolationHardeningMethod::integrate_via_evol_eqs:
      {
        interp_plastic_strain = integrate_plastic_strain(
            ViscoplastUtils::HardeningIntegrationInput{
                .interp_equiv_stress = state_quantities_stress.curr_equiv_stress,
    ...
    if (err_status == ViscoplastUtils::ErrorType::no_errors)
    {
      if (local_timint_analysis_.has_value() && adaptive_estimate_interp_manager_.has_value() &&
          local_timint_analysis_->track_quantities())
      {
        local_timint_analysis_->increment_aei_num_interp_iters(
            gp_, adaptive_estimate_interp_manager_->num_estimate_interp_iters());
      }
    ...
    else
    {
      ...
      // interval
      if (adaptive_estimate_interp_manager_->upper_interp_bound(gp_) <= 1.0e-8)
      {
        err_status = ViscoplastUtils::ErrorType::no_errors;
        adaptive_estimate_interp_manager_->disable_further_reestimations();
        adaptive_estimate_interp_manager_->reset_num_estimate_interp_iters();
        return wrap_unknowns(
            deftensors.elastic_predictor_inverse_plastic_defgrad, last_plastic_strain);
      }
    }
  }
    ...
    InelasticDefgradTransvIsotropElastViscoplastUtils::EvaluationAction& eval_action)
{
  ...
  ViscoplastUtils::ErrorType err_status{ViscoplastUtils::ErrorType::no_errors};
  Core::LinAlg::Matrix<10, 1> updated_estimate =
      interpolate_estimate(dt, deftensors, last_plastic_strain, err_status);
...
Mat::InelasticDefgradTransvIsotropElastViscoplast::reestimate_to_restart_local_newton(
    ...
    const Mat::InelasticDefgradTransvIsotropElastViscoplastUtils::
        LocalIntegrationDeformationTensors& deftensors,
    const double last_plastic_strain, ViscoplastUtils::EvaluationAction& eval_action)
{
  if (!adaptive_estimate_interp_manager_.has_value())
  {
    FOUR_C_THROW("{}",
  ...
  // check whether re-estimation is still possible
  if (!adaptive_estimate_interp_manager_->is_reestimation_possible(gp_))
  {
  ...
  // verify whether we need to verify the intermediate point as a viable updated estimate; we do
  // this based on the current interval
  // \f$ \xi - \xi_{\text{E}} \f$
  const double reestimation_interval =
      adaptive_estimate_interp_manager_->current_interp_point(gp_) -
      adaptive_estimate_interp_manager_->lower_interp_bound(gp_);


  if (reestimation_interval <
      parameter()->adaptive_estimate_interp_params().min_reestimation_interval)
  {
    // set lower <- current, and return re-interpolated estimate using these updated bounds
    adaptive_estimate_interp_manager_->set_lower_interp_bound_to_current_interp_point(gp_);

    return reinterpolate_with_updated_bounds(dt, deftensors, last_plastic_strain, eval_action);
  }

  // verify whether the intermediate point is a viable updated estimate
  adaptive_estimate_interp_manager_->set_current_interp_point(gp_,
      ViscoplastUtils::AdaptiveEstimateInterpolationManager::CurrentInterpPointPreset::
          intermediate_point);
  ...
  ViscoplastUtils::ErrorType err_status{ViscoplastUtils::ErrorType::no_errors};
  ViscoplastUtils::StateQuantities interp_state_quantities =
      evaluate_state_quantities(deftensors.right_cg, interp_iFin, 0.0, err_status, dt,
          ViscoplastUtils::StateQuantityEvalType::equiv_stress_only);
  if (viscoplastic_law_->evaluate_stress_ratio(
          interp_state_quantities.curr_equiv_stress, last_plastic_strain) < 1.0)
  {
    err_status = ViscoplastUtils::ErrorType::under_yield_surface;
  }
  ...
  if (err_status != ViscoplastUtils::ErrorType::no_errors)
  {
    switch (err_status)
    {
      ...
      {
        // set lower <- current (=intermediate!), and return re-interpolated estimate using these
        // updated bounds
        adaptive_estimate_interp_manager_->set_lower_interp_bound_to_current_interp_point(gp_);

        return reinterpolate_with_updated_bounds(dt, deftensors, last_plastic_strain, eval_action);
      }
      case ViscoplastUtils::ErrorType::under_yield_surface:
      {
        // set upper <- current (=intermediate!), and return re-interpolated estimate using these
        // updated bounds
        adaptive_estimate_interp_manager_->set_upper_interp_bound_to_current_interp_point(gp_);

        return reinterpolate_with_updated_bounds(dt, deftensors, last_plastic_strain, eval_action);
      }
      default:
        FOUR_C_THROW("{}", get_error_warning_info(std::format(
                               "Error type {} not supported for re-estimation", err_status)));
    }
  }
      ...
      if (err_status != InelasticDefgradTransvIsotropElastViscoplastUtils::ErrorType::no_errors)
      {
        switch (err_status)
        {
          case ViscoplastUtils::ErrorType::overflow_error:
          {
            ...
            return reinterpolate_with_updated_bounds(
                dt, deftensors, last_plastic_strain, eval_action);
          }
          case ViscoplastUtils::ErrorType::under_yield_surface:
          {
            adaptive_estimate_interp_manager_->set_upper_interp_bound_to_current_interp_point(gp_);
          ...
          default:
            FOUR_C_THROW("{}",
                get_error_warning_info(
                    std::format("Error type {} not supported after failed hardening integration!",
                        err_status)));
        }
      }
      break;
  ...
  if (err_status == InelasticDefgradTransvIsotropElastViscoplastUtils::ErrorType::no_errors)
  {
    if (local_timint_analysis_.has_value() && local_timint_analysis_->track_quantities())
    {
      // we explicitly only track this successful proceeding with the intermediate point, since
      // all increases of lower bound and reinterpolations are implicitly tracked via
      // interpolate_estimate
      local_timint_analysis_->append_estimate_interp_tracking_data(gp_,
          {.current_interp_point = adaptive_estimate_interp_manager_->current_interp_point(gp_),
              .lower_interp_bound = adaptive_estimate_interp_manager_->lower_interp_bound(gp_),
              .upper_interp_bound = adaptive_estimate_interp_manager_->upper_interp_bound(gp_),
...
void Mat::InelasticDefgradTransvIsotropElastViscoplast::
    update_adaptive_estimate_interp_starting_points(const unsigned int gp)
{
  if (!adaptive_estimate_interp_manager_.has_value())
  {
    FOUR_C_THROW("{}",
        get_error_warning_info(
            "This method should not be called without enabling adaptive estimate interpolation!"));
  }

  const ViscoplastUtils::AdaptiveEstimateInterpolationParams& aei_params =
      parameter()->adaptive_estimate_interp_params();
  ...
  switch (aei_params.starting_point_type)
  {
    ...
    case ViscoplastUtils::AdaptiveEstimateInterpolationStartingPointType::last_interpolation_point:
    {
    ...
    {
      adaptive_estimate_interp_manager_->set_starting_point(gp, get_input_optimal_equiv_stress(gp));
      break;
    }
    default:
...
/*--------------------------------------------------------------------*
 *--------------------------------------------------------------------*/
ViscoplastUtils::OptimalEquivStressStartingPointInput
Mat::InelasticDefgradTransvIsotropElastViscoplast::get_input_optimal_equiv_stress(
    const unsigned int gp)
{
  // get deformation gradient components
  ViscoplastUtils::LocalIntegrationDeformationTensors deftensors(
      time_step_quantities_.current_defgrad[gp],
      time_step_quantities_.last_plastic_defgrad_inverse[gp]);

  ViscoplastUtils::ErrorType err_status{ViscoplastUtils::ErrorType::no_errors};

  // compute solution stress at the considered Gauss point
  ViscoplastUtils::StateQuantities state_quantities_sol =
      evaluate_state_quantities(deftensors.right_cg,
          time_step_quantities_.current_plastic_defgrad_inverse[gp], -1.0, err_status, -1.0,
          ViscoplastUtils::StateQuantityEvalType::
              equiv_stress_only);  // we use only dummy values for quantities that are not
                                   // required in this partial computation
  if (err_status != ViscoplastUtils::ErrorType::no_errors)
  {
    FOUR_C_THROW(
  ...
  // point
  ViscoplastUtils::StateQuantities state_quantities_elast_pred =
      evaluate_state_quantities(deftensors.right_cg,
          time_step_quantities_.last_plastic_defgrad_inverse[gp], -1.0, err_status, -1.0,
          ViscoplastUtils::StateQuantityEvalType::
              equiv_stress_only);  // we use only dummy values for quantities that are not
                                   // required in this partial computation
  ...
  Core::LinAlg::Matrix<3, 3> iFin_plastic_pred =
      adaptive_estimate_interp_manager_->get_inverse_inelastic_defgrad_plastic_pred(
          gp, deftensors.inv_defgrad);
  ViscoplastUtils::StateQuantities state_quantities_plast_pred =
      evaluate_state_quantities(deftensors.right_cg, iFin_plastic_pred, -1.0, err_status, -1.0,
          ViscoplastUtils::StateQuantityEvalType::
              equiv_stress_only);  // we use only dummy values for quantities that are not
                                   // required in this partial computation
...
Mat::InelasticDefgradTransvIsotropElastViscoplast::OverallState
Mat::InelasticDefgradTransvIsotropElastViscoplast::save_overall_state() const
{
  return {.time_step_quantities = time_step_quantities_,
      .state_quantities = state_quantities_,
      .state_quantity_derivatives = state_quantity_derivatives_,
      .viscoplastic_law_state = viscoplastic_law_->get_state(),
      .local_substepping_utils = local_substepping_utils_,
      .local_newton_manager_state = local_newton_manager_.get_state(),
      .adaptive_estimate_interp_manager_state = adaptive_estimate_interp_manager_->get_state()};
}


void Mat::InelasticDefgradTransvIsotropElastViscoplast::reinstate_overall_state(
    const InelasticDefgradTransvIsotropElastViscoplast::OverallState& overall_state)
{
  time_step_quantities_ = overall_state.time_step_quantities;
  state_quantities_ = overall_state.state_quantities;
  state_quantity_derivatives_ = overall_state.state_quantity_derivatives;
  viscoplastic_law_->reinstate_state(*overall_state.viscoplastic_law_state);
  local_substepping_utils_ = overall_state.local_substepping_utils;
  local_newton_manager_.reinstate_state(overall_state.local_newton_manager_state);
  adaptive_estimate_interp_manager_->reinstate_state(
      overall_state.adaptive_estimate_interp_manager_state);
}

void Mat::InelasticDefgradTransvIsotropElastViscoplast::init_local_timint_analysis()
{
  FOUR_C_ASSERT_ALWAYS(!parameter()->use_local_substepping(),
      "Analysis of local time integration not yet enabled / tested for local substepping");

  // determine whether the element ids match
  if (ele_gid_ != parameter()->local_timint_analysis_params().ele_gid) return;

  // only proceed on the rank that owns this element
  const auto discretization = Global::Problem::instance()->get_dis("structure");
  FOUR_C_ASSERT_ALWAYS(discretization != nullptr, "Structure discretization not found.");

  if (discretization->element_row_map()->lid(static_cast<int>(ele_gid_)) < 0) return;

  // initialize analysis framework if enabled by the user exactly once, on the owning rank of the
  // element!
  if (!local_timint_analysis_.has_value())
  {
    local_timint_analysis_.emplace(parameter()->use_adaptive_estimate_interp(), num_gp_);
  }
}
