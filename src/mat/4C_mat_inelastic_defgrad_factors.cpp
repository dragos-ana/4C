    ...
    ViscoplastUtils::ErrorType& err_status)
{
  ...
  while (true)
  {
    ...
    if (!adaptive_estimate_interp_manager_->is_plastic_pred_construct_possible())
    {
      // throw error
      FOUR_C_THROW("{}", get_error_warning_info(std::format(
    ...
    InelasticDefgradTransvIsotropElastViscoplastUtils::ErrorType& err_status) const
{
  ...
  FOUR_C_ASSERT(adaptive_estimate_interp_manager_.has_value(), "{}",
      get_error_warning_info(
          std::format("This method  should not be called without initializing ")));
  ...
  while (true)
  {
    ...
    if (err_status == ViscoplastUtils::ErrorType::overflow_error)
      return -1;
    else if (err_status != ViscoplastUtils::ErrorType::no_errors)
    {
      FOUR_C_THROW("Error: {}",
    ...
    const Core::LinAlg::Matrix<3, 3>& iFin_candidate, const double plastic_strain_candidate)
{
  ...
  // verify whether plastic strain is exhibited
  const bool exhibit_plastic_strain =
      (state_quantities_.curr_equiv_plastic_strain_rate * local_integration_input.step >
          InelasticDefgradTransvIsotropElastViscoplastUtils::zero_plastic_strain_increment);
  if (!exhibit_plastic_strain) return ViscoplastUtils::ErrorType::under_yield_surface;
    ...
    InelasticDefgradTransvIsotropElastViscoplastUtils::ErrorType& err_status)
{
  ...
  while (true)
  {
    ...
    if (!adaptive_estimate_interp_manager_->is_estimate_interp_possible())
    {
      FOUR_C_THROW("{}",
          get_error_warning_info(std::format(
    ...
    // gradient
    ViscoplastUtils::StateQuantities state_quantities_stress = evaluate_state_quantities(
    ...
    ViscoplastUtils::EvaluationAction& eval_action)
{
  ...
  // if the stress state is under the (previous) yield surface -> set upper bound to intermediate
  // point to shift interpolation towards the elastic predictor and work with this updated interval
  if (viscoplastic_law_->uses_yield_surface() &&
