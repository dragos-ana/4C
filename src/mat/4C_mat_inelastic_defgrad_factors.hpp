  ...
  class InelasticDefgradTransvIsotropElastViscoplast : public InelasticDefgradFactors
  {
   ...
   private:
    ...
    struct PredInterpFactors
    {
      ...
      //! struct holding relevant tracking data for the output of the
      //! predictor adaptation when writing to a csv
      //! file
      ...
      struct CSVOutputMicroIterData
      {
        CSVOutputMicroIterData()
        {
          // initialize default micro iteration data collector
          MicroIterDataCollector mi_data_collector{};

          for (unsigned int mi = 0; mi < MAX_NUM_PRED_ADAPT_ITERS; ++mi)
          {
            all_microiter[mi] = -1;
            all_current_xi[mi] = mi_data_collector.current_xi;
            all_current_equiv_stress[mi] = mi_data_collector.current_equiv_stress;
            all_current_plastic_strain[mi] = mi_data_collector.current_plastic_strain;
            all_current_error_status[mi] = mi_data_collector.current_error_status;
          }
        }
        ...
        //! adaptation)
        std::array<unsigned int, MAX_NUM_PRED_ADAPT_ITERS> all_microiter;

        //! current interpolation factors \f$ \xi \f$ of all
        //! microiterations
        std::array<double, MAX_NUM_PRED_ADAPT_ITERS> all_current_xi;

        //! current equivalent stresses of all microiterations (associated
        //! with the current interpolation factors)
        std::array<double, MAX_NUM_PRED_ADAPT_ITERS> all_current_equiv_stress;

        //! current plastic strains of all microiterations (associated
        //! with the current interpolation factors)
        std::array<double, MAX_NUM_PRED_ADAPT_ITERS> all_current_plastic_strain;

        //! current error status of all microiterations (associated with
        //! the current interpolation factors)
        std::array<InelasticDefgradTransvIsotropElastViscoplastUtils::ErrorType,
            MAX_NUM_PRED_ADAPT_ITERS>
            all_current_error_status;
        ...
        {
          all_microiter[micro_iter] = micro_iter;
          all_current_xi[micro_iter] = mi_data_collector.current_xi;
          all_current_equiv_stress[micro_iter] = mi_data_collector.current_equiv_stress;
          all_current_plastic_strain[micro_iter] = mi_data_collector.current_plastic_strain;
          all_current_error_status[micro_iter] = mi_data_collector.current_error_status;
        }
      };

      //! writes data from each microiteration of a single predictor
      //! adaptation (specified via tracking data) to a dedicated csv file
      void write_pred_adapt_micro_iter_data_to_csv(CSVOutputTrackingData csv_output_tracking_data,
          CSVOutputMicroIterData csv_output_micro_iter_data)
      {
        ...
        Core::IO::RuntimeCsvWriter csv_writer{my_rank,
            *Global::Problem::instance()->output_control_file(),
            "pred-adapt-micro-iter-output-ele-gid-" +
                ...
                std::to_string(csv_output_tracking_data.globiter_or_timestep_index_) +
                "-lnl-iter-" + std::to_string(csv_output_tracking_data.lnl_iter)};
        ...
        for (unsigned int mi = 0; mi < MAX_NUM_PRED_ADAPT_ITERS; ++mi)
        {
          // DEBUG
          std::cout << "-------------------------------------------" << std::endl;
          std::cout << "current_microiter: " << csv_output_micro_iter_data.all_microiter[mi]
                    << std::endl;
          std::cout << "current_xi: " << csv_output_micro_iter_data.all_current_xi[mi] << std::endl;
          std::cout << "current_err_status: "
                    << EnumTools::enum_name(csv_output_micro_iter_data.all_current_error_status[mi])
                    << std::endl;
          std::cout << "current_equiv_stress: "
                    << csv_output_micro_iter_data.all_current_equiv_stress[mi] << std::endl;
          std::cout << "current_plastic_strain: "
                    << csv_output_micro_iter_data.all_current_plastic_strain[mi] << std::endl;
          std::cout << "-------------------------------------------" << std::endl;
