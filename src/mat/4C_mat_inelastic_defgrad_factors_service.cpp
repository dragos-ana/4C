...
namespace
{
  ...
  void precondition_matrix(Core::LinAlg::Matrix<3, 3>& matrix, const double tol)
  {
...
/*--------------------------------------------------------------------*
 *--------------------------------------------------------------------*/
AEINamespace::PredictorInterpolator::PredictorInterpolator()
...
/*--------------------------------------------------------------------*
 *--------------------------------------------------------------------*/
void AEINamespace::PredictorInterpolator::resize(const unsigned int numgp)
{
...
/*--------------------------------------------------------------------*
 *--------------------------------------------------------------------*/
void AEINamespace::PredictorInterpolator::pack(Core::Communication::PackBuffer& data) const
{
...
/*--------------------------------------------------------------------*
 *--------------------------------------------------------------------*/
void AEINamespace::PredictorInterpolator::unpack(Core::Communication::UnpackBuffer& buffer)
{
...
/*--------------------------------------------------------------------*
 *--------------------------------------------------------------------*/
void AEINamespace::PredictorInterpolator::construct_prelim_plastic_pred(const unsigned int gp,
    const Core::LinAlg::Matrix<3, 3>& elastic_defgrad_elastic_pred,
    const AEINamespace::AEIParams& aei_params)
{
  // consistency checks
  ...
  switch (aei_params.plastic_predictor_construction.elastic_stretch_eigenvect_type)
  {
    case AEINamespace::PrelimPlasticPredictor::ElasticStretchEigenvectType::from_elastic_predictor:
    {
    ...
    default:
    {
      ...
      FOUR_C_THROW(
          ...
          "predictor",
          EnumTools::enum_name(
              aei_params.plastic_predictor_construction.elastic_stretch_eigenvect_type));
    }
  }

  // elastic rotation
  switch (aei_params.plastic_predictor_construction.elastic_rotation_type)
  {
    case AEINamespace::PrelimPlasticPredictor::ElasticRotationType::from_elastic_predictor:
    {
    ...
    default:
    {
      ...
      FOUR_C_THROW("Elastic rotation type {} not yet enabled for the plastic predictor!",
          EnumTools::enum_name(aei_params.plastic_predictor_construction.elastic_rotation_type));
    }
  }

  // elastic stretch eigenvalues
  const double detF = precond_elastic_defgrad_elastic_pred.determinant();
  switch (aei_params.plastic_predictor_construction.elastic_stretch_eigenval_type)
  {
    case AEINamespace::PrelimPlasticPredictor::ElasticStretchEigenvalType::scale_unit:
    {
    ...
    default:
    {
      FOUR_C_THROW("Elastic stretch eigenvalue type {} not yet enabled for the plastic predictor",
          EnumTools::enum_name(
              aei_params.plastic_predictor_construction.elastic_stretch_eigenval_type));
    }
  }
...
/*--------------------------------------------------------------------*
 *--------------------------------------------------------------------*/
void AEINamespace::PredictorInterpolator::interpolate_elastic_defgrad_contributions(
    const unsigned int gp, const double interp_loc, Core::LinAlg::Matrix<4, 1>& interp_rel_rot_quat,
    std::vector<double>& interp_eigenval,
    Core::LinAlg::Matrix<4, 1>& interp_rel_eigenvect_rot_quat) const
{
  // consistency checks
  FOUR_C_ASSERT(0.0 <= interp_loc && interp_loc <= 1.0,
      "Interpolation is constrained to the [0, 1] interval, with 0 specifying the elastic "
      "predictor and 1 specifying the plastic predictor! The current plastic predictor location is "
      "{}",
      interp_loc);
  FOUR_C_ASSERT(gp < eigenval_elast_pred_.size(),
      "Inconsistent Gauss point index {}, with set Gauss point size {}", gp,
  ...
  FOUR_C_ASSERT(interp_eigenval.size() == 3, "The number of eigenvalues {} must actually be == 3",
      interp_eigenval.size());
...
/*--------------------------------------------------------------------*
 *--------------------------------------------------------------------*/
Core::LinAlg::Matrix<3, 3> AEINamespace::PredictorInterpolator::interpolate_elastic_defgrad(
...
/*--------------------------------------------------------------------*
 *--------------------------------------------------------------------*/
void AEINamespace::PredictorInterpolator::set_plastic_predictor_after_construction_algo(
    const unsigned int gp, const double plastic_pred_loc)
{
  // consistency checks
  FOUR_C_ASSERT(0.0 <= plastic_pred_loc && plastic_pred_loc <= 1.0,
      "Interpolation is constrained to the [0, 1] interval, with 0 specifying the elastic "
      "predictor and 1 specifying the plastic predictor! The current plastic predictor location is "
      "{}",
      plastic_pred_loc);
...
AEINamespace::InterpolationPointContainer::InterpolationPointContainer(
    const Mat::InelasticDefgradTransvIsotropElastViscoplastUtils::AdaptiveEstimateInterpolation::
        AEIParams& aei_params)
{
  ...
  switch (aei_params.estimate_interpolation.starting_point_type)
  {
    ...
    {
      starting_points.resize(1, aei_params.estimate_interpolation.user_set_starting_point);
      break;
    }
    default:
    {
      starting_points.resize(1, aei_params.estimate_interpolation.interval_scanning_param);
      break;
    }
  }
...
/*--------------------------------------------------------------------*
 *--------------------------------------------------------------------*/
void AEINamespace::InterpolationPointContainer::reset_bounds_and_current_interp_point(
    const unsigned int gp)
{
  // consistency checks
...
/*--------------------------------------------------------------------*
 *--------------------------------------------------------------------*/
void AEINamespace::InterpolationPointContainer::resize(const unsigned int numgp)
{
...
/*--------------------------------------------------------------------*
 *--------------------------------------------------------------------*/
void AEINamespace::InterpolationPointContainer::pack(Core::Communication::PackBuffer& data) const
{
...
/*--------------------------------------------------------------------*
 *--------------------------------------------------------------------*/
void AEINamespace::InterpolationPointContainer::unpack(Core::Communication::UnpackBuffer& buffer)
{
