...
namespace
{
  ...
  AEINamespace::InterpolationShiftAction get_interpolation_shift_action(ErrorType error_type)
  {
    switch (error_type)
    {
...
void AEINamespace::PredictorInterpolator::construct_prelim_plastic_pred(const unsigned int gp,
    ...
    const PreconditioningSettings& preconditioning_settings,
    const PlasticPredictorConstructionParams& plastic_predictor_construction_params)
{
  ...
  if (preconditioning_settings.precondition_elastic_pred)
  {
    precond_elastic_defgrad_elastic_pred = precondition_matrix(
        elastic_defgrad_elastic_pred, preconditioning_settings.tol_precondition_elastic_pred);
  }
    ...
    const EstimateInterpolationParams& estimate_interpolation_params)
{
  ...
  switch (estimate_interpolation_params.starting_point_type)
  {
    ...
    {
      starting_points.resize(1, estimate_interpolation_params.user_set_starting_point);
      break;
    }
    default:
    {
    ...
    const unsigned int gp, const LocalIntegrationInput& local_integration_input)
{
  ...
  predictor_interpolator_.construct_prelim_plastic_pred(gp,
      local_integration_input.elastic_predictor_elastic_defgrad, params_.preconditioning,
    ...
    const unsigned int gp, const ErrorType& eval_err_type)
{
  FOUR_C_ASSERT_ALWAYS(eval_err_type != ErrorType::no_errors,
      ...
      EnumTools::enum_name(eval_err_type));
  ...
  switch (interp_shift_action)
  {
    ...
    {
      set_upper_interp_bound_to_current_interp_point(gp);
      break;
    }
    case InterpolationShiftAction::shift_towards_plastic_pred:
    {
      set_lower_interp_bound_to_current_interp_point(gp);
      break;
    }
    default:
    {
...
/*--------------------------------------------------------------------*
 *--------------------------------------------------------------------*/
void AEINamespace::AEIManager::set_user_starting_point(const unsigned gp)
{
  ...
  FOUR_C_ASSERT_ALWAYS(params_.estimate_interpolation.starting_point_type ==
                           AEINamespace::StartingPointType::user_set,
...
/*--------------------------------------------------------------------*
 *--------------------------------------------------------------------*/
void AEINamespace::AEIManager::set_stress_based_starting_point(
